...
#Tomorrow (7/1): update code below the revised filter_loc1 (called filter_loc1_revised)
#to align with the revised filter_loc1. Once I make sure that this revised version
#properly calculates EE then I can delete the secion indicated for deletion
...
filter_loc1 <-sable_dwn %>%
  ...
  group_by(ID, SABLE) %>% 
  mutate(
    zt_time = zt_time(DateTime), #changed from hr
    is_zt_init = replace_na(as.numeric(DateTime!=lag(DateTime)), 0), #changed from hr
...
mutate(LM_complete_day = case_when(
  ...
  zt_time %in% c(0, 1, 2, 3, 4) & sable_idx=="SABLE_DAY_7" ~ 3,
  zt_time>4 & zt_time<24 & sable_idx=="SABLE_DAY_8" ~ 3,
  
  zt_time %in% c(0, 1, 2, 3, 4) & sable_idx=="SABLE_DAY_8" ~ 4,
  zt_time>4 & zt_time<24 & sable_idx=="SABLE_DAY_9" ~ 4))

# End DELETE--------------------####

# Start KEEP : Possible replacement for above version of filter_loc1 ####
...
filter_loc1_revised <-sable_dwn %>%
  ...
  mutate( #I think using this code instead of what is below fixes the 47hr day issue. Do the checks though. Also, I am still missing data for 6-20 ofr 3749 and 3753
    zt_time = zt_time(DateTime),
    is_zt_init = replace_na(as.numeric(zt_time == 0 & lag(zt_time, default = -1) != 0), 0),
    complete_days = cumsum(is_zt_init)
  ) %>%
  #mutate(
    #zt_time = zt_time(DateTime), 
        #uses function above to calculate zt time using DateTime column
   # is_zt_init = replace_na(as.numeric(zt_time == 0 & lag(zt_time) == 23), 0), 
        #lag(zt_time) looks at the previous row's zt_time and checks if the previous row was at the end of the previous zt cycle
        #zt_time==0 asks: TRUE or FALSE, is the current row at ZT0 (i.e. start of a new zt cycle)?
        #if the previous row was the end of the previous zt cycle AND the current row is the start of 
        #a new ZT cycle then is_zt_init is 1. is_zt_init is zero when there is a transition 
        #between zt cycles (when lights turn off)
   # complete_days = cumsum(is_zt_init)) %>%
        #indicates which of the complete days the measurement was taken on
  ...
  mutate(
    recording_duration = as.numeric(difftime(max(DateTime),
                                             min(DateTime),
                                             units="hours")),
...
filter_loc1 %>% 
  ...
  filter(SABLE=="BW loss")%>% #All time points other than BW loss look good from this relatively macro level
  summarise(n_each_ID = n_distinct(hr)) %>%
  print(n=57) %>%
  filter(n_each_ID== 24) %>%
  print(n=42)
...
filter_TEE1 <-sable_dwn %>%
  ...
  mutate(
    ...
    complete_days = cumsum(is_zt_init), 
    #indicates which of the complete days the measurement was taken on
    is_complete_day = complete_days >0 #removes partial days
  ) %>%
  filter(is_complete_day==TRUE) %>%
  ungroup ()%>%
  group_by(ID, complete_days, SABLE)
