#Goal of script: Graph and do stats for NZO Lean mass

# Revised: 11-7-25

#Need to run script to make sable_TEE_RMR_adj to use as source data for fat free mass (Lean)
#Located at the bottom of this script if it is not already in the Environment

# Fat free mass: ANOVA, T-test, linear mixed model and emmeans, bar graph
...
####----- Graphs of fat free mass at the 5 time points (n=16 NZO mice) -----####
...
#####----- Barplot - NZO fat free mass (measured values from echoMRI_data/sable_TEE_adj_RMR) ####
Lean_barplot_measured <- sable_TEE_adj_RMR %>%
  ggplot(aes(x = SABLE, y = Lean, fill = GROUP)) + 
  ...
  scaleColor +
  theme_minimal() +
  labs(title = "NZO fat free mass", 
       y = "Fat free mass (grams)", 
       x= "Time point", 
       color = "Treatment group",
       fill = "Treatment group") +
  #scale_y_continuous( # set y-axis breaks every 10 grams
  #breaks = seq(0, max(sable_TEE_adj_RMR$Lean, na.rm = TRUE), by = 10)) +
  format.plot +
  theme(
    legend.position = "top",
...
Lean_barplot_measured

##### Scatterplot -NZO fat free mass (measured values from echoMRI_data/sable_TEE_adj_RMR)####
Lean_scatterplot_measured <- sable_TEE_adj_RMR %>%
  ggplot(aes(x = SABLE, y = Lean, color = GROUP)) + 
  #geom_point(
  ...
  labs(title = "NZO fat free mass", 
    ...
    color = "Treatment group") +
  ylim(20,35)+
  #scale_y_continuous(breaks = seq(0, max(sable_TEE_adj_RMR$Lean, na.rm = TRUE), by = 10)) +
  format.plot +
  theme(legend.position = "top",
    axis.ticks.y = element_line(),
    axis.text.x = element_text(angle = 45, hjust = 1),
    plot.title = element_text(hjust = 0.5, face = "bold"))
Lean_scatterplot_measured
...
ttest_results_Lean <- sable_TEE_adj_RMR %>%
  group_by(SABLE) %>%
  t_test(Lean ~ GROUP, var.equal = TRUE) %>%   # or var.equal = FALSE if not assumed
...
anova_results_Lean <- sable_TEE_adj_RMR %>%
  ungroup() %>%
  group_by(GROUP) %>%
  anova_test(dv = Lean, wid = ID, within = SABLE)
anova_results_Lean

#Post-hoc test for ANOVA
pairwise_results_Lean <- sable_TEE_adj_RMR %>%
  group_by(GROUP) %>%
  pairwise_t_test(Lean ~ SABLE, paired = TRUE, p.adjust.method = "bonferroni") %>%
  filter(p.adj.signif <= 0.05)
pairwise_results_Lean

####----- Linear mixed model, emmeans, contrasts -----####
# Build linear mixed model
    model_Lean <- lmer(Lean ~ SABLE * GROUP + (1 | ID), data = sable_TEE_adj_RMR)
    summary(model_Lean)
...
# Pairwise contrasts within each GROUP
    ...
    contrasts_Lean_df <- as.data.frame(contrasts_by_group_Lean)
    contrasts_Lean_df
    #Filter for significant contrasts
    Sig_contrasts_Lean <- contrasts_Lean_df %>%
      filter(p.value <=0.05)
    Sig_contrasts_Lean
...
# Pairwise contrasts within each SABLE (time point)
    ...
    # Convert to a data frame
    contrasts_SABLE_Lean_df <- as.data.frame(contrasts_by_SABLE_Lean)
    contrasts_SABLE_Lean_df
...
barplot_emm_Lean <- emm_Lean_df %>%
  ...
  theme_minimal() +
  labs(
    title = "NZO Fat free mass (emmeans)",
    y = "Fat free mass (grams)",
    x = "Time point",
    fill = "Treatment group") +
  format.plot +
