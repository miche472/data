...
#---
#---
#Bar graph- measured daily FI (grams measured manually each day) ####
#prepare df
Exp2_tracker_days <- Exp2_tracker %>%
  group_by(ID) %>%
  mutate(
    first_inj = DATE[COMMENTS %in% c("INJECT_DAY_2_DOSE_ONE")][1], 
    days_from_first = as.integer(DATE - first_inj),
    Relative_day = floor(days_from_first)) %>%  # Relative day
  ungroup()

##Bar graph ####
filter_exp2_tracker_days <- Exp2_tracker_days %>%
  filter(!(Relative_day > 14 & DRUG=="Vehicle")) %>% #these mice started BW loss in exp 2a wk 2 (were rolled into Exp 2b)
  filter(Relative_day > -10) %>% 
  filter(Relative_day < 20) %>%
  filter(!(ID=="3747")) #Filter out ID=="3747" because this mouse didn't undergo 2 weeks of weight loss

plot_Day_FI_exp2a <- ggplot(filter_exp2_tracker_days, aes(x=Relative_day , y=corrected_intake_gr, group=DRUG, fill=DRUG, color=DRUG)) +
  geom_bar(stat = "summary", 
           fun = "mean", 
           position = position_dodge(width = 0.8), width=0.73) +
  geom_errorbar(stat = "summary", 
  ...
  labs(x="Day relative to first injection",
       y= "Daily intake (g)",
       title= "Daily food intake (grams/day)",
       color="Treatment", fill="Treatment")
plot_Day_FI_exp2a
