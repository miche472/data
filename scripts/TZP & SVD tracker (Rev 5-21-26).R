...
mutate(
    ...
    Treatment_day =case_when(COMMENTS=="INJECT_DAY_ONE"~0, # All days since treatment started (including days w/o injection)
                          ...
                          COMMENTS=="INJECT_DAY_ELEVEN_DOSE_TWO"~28,  #Fri 5/15
                          COMMENTS=="INJECT_DAY_TWELVE_DOSE_TWO"~29,  #Sat 5/16
                          COMMENTS=="INJECT_DAY_ONE_DOSE_THREE"~30,   #Sun 5/17
                          COMMENTS=="INJECT_DAY_TWO_DOSE_THREE"~31,   #Mon 5/18
                          COMMENTS=="INJECT_DAY_THREE_DOSE_THREE"~32, #Tues 5/19
                          COMMENTS=="INJECT_DAY_FOUR_DOSE_THREE"~33,  #Wed 5/20
                          COMMENTS=="INJECT_DAY_FIVE_DOSE_THREE"~34),#Thurs 5/21
    ...
    Inject_day= case_when(COMMENTS=="INJECT_DAY_ONE"~0, # Only days during treatment when an injection was given
                          ...
                          COMMENTS=="INJECT_DAY_ONE_DOSE_THREE"~30,   
                          COMMENTS=="INJECT_DAY_TWO_DOSE_THREE"~31,   
                          COMMENTS=="INJECT_DAY_THREE_DOSE_THREE"~32, 
                          COMMENTS=="INJECT_DAY_FOUR_DOSE_THREE"~33,  
                          COMMENTS=="INJECT_DAY_FIVE_DOSE_THREE"~34),
    Dose_step = case_when(ID %in% c(3730, 3731, 3732, 3733, 3735, 3736, 3737, 3738, 3739, 3740, 3741)
                           ...
                           ID %in% c(3730, 3731, 3732, 3733, 3735, 3736, 3737, 3738, 3739, 3740, 3741) 
                           & STAGE == "BW loss" & 
                           DATE> "2026-5-02" & DATE< "2026-05-17" & DRUG == "Survodutide" ~ "40nmol/kg",
                          
                           ID %in% c(3730, 3731, 3732, 3733, 3735, 3736, 3737, 3738, 3739, 3740, 3741)
                           & STAGE == "BW loss" & DATE >"2026-5-16" & DRUG == "Survodutide" ~ "50nmol/kg",
                          ...
                          ID %in% c(3730, 3731, 3732, 3733, 3735, 3736, 3737, 3738, 3739, 3740, 3741)
                          & STAGE == "BW loss" & DATE >"2026-5-16" & DRUG == "Tirzepatide" ~ "30nmol/kg",
...
#filter_out(ID == "3731") #Try this since 3732 has a much lower BW and looks like an outlier
...
SVD_plot_pct_BW_BWloss<-ggplot(SVD_GLP1_BWloss_plot, aes(x=Treatment_day, y=BW_pct_change, group=DRUG, fill=DRUG, color=DRUG)) +
  ...
  format.plot+
  geom_hline(yintercept=0) + 
  geom_vline(xintercept=16, linetype="dotted", linewidth=1, color="#62748E")+ #increased dose (30nmol/kg-->40nmol/kg)
  geom_vline(xintercept=20, linetype="dashed", linewidth=1, color="#62748E")+ #started doing injections every other day 
  geom_vline(xintercept=23, linetype="dashed", linewidth= 1, color="#62748E")+ #final day of every other day injections 
  geom_vline(xintercept=30, linetype="dotted", linewidth=1, color="#62748E")+ #increased dose (40nmol/kg--> 50nmol/kg)
  labs(x="Day",
       y= "ΔBW (%) ",
...
SVD_Incretin_FI_daily_kcal_plot<-ggplot(SVD_GLP1_BWloss_plot, aes(x=Treatment_day, y=corrected_intake_kcal, group=DRUG, fill=DRUG, color=DRUG)) +
  geom_point(stat = "summary", fun = "mean", aes(color=DRUG), size=4) +
  stat_summary(fun = mean,geom = "label",aes(label = round(after_stat(y), 1)), vjust = -3, size = 4, color = "black", # text color
               fill = "white", # label background
               label.padding = unit(0.15, "lines")) + #this seems to make the text boxes smaller but doesn't change text size
  geom_line(stat = "summary", fun = "mean", aes(color=DRUG), linewidth=2) +
  geom_point(aes(group=ID), shape=1) + #this adds individual values from each ID to the graph
  geom_line(aes(group=ID), alpha=0.6) +
  geom_errorbar(stat = "summary", fun.data = mean_se, aes(width=0.08), width=0.45, linewidth=1, color="black") +
...
TZP_plot_pct_BW_BWloss<-ggplot(TZP_GLP1_BWloss_plot, aes(x=Treatment_day, y=BW_pct_change, group=DRUG, fill=DRUG, color=DRUG)) +
  ...
  geom_vline(xintercept=20, linetype="dashed", linewidth=1, color="#62748E")+ #started doing injections every other day 
  geom_vline(xintercept=23, linetype="dashed", linewidth= 1, color="#62748E")+ #final day of every other day injections 
  geom_vline(xintercept=30, linetype="dotted", linewidth=1, color="#62748E")+ #increased dose (20nmol/kg--> 30nmol/kg)
  labs(x="Day",
       y= "ΔBW (%) ",
...
Veh_plot_pct_BW_BWloss<-ggplot(Veh_GLP1_BWloss_plot, aes(x=Treatment_day, y=BW_pct_change, group=DRUG, fill=DRUG, color=DRUG)) +
  ...
  format.plot+
  geom_hline(yintercept=0) + 
  geom_vline(xintercept=16, linetype="dotted", linewidth=1, color="#62748E")+ #increased dose for TZP & SVD
  geom_vline(xintercept=20, linetype="dashed", linewidth=1, color="#62748E")+ #started doing injections every other day 
  geom_vline(xintercept=23, linetype="dashed", linewidth= 1, color="#62748E")+ #final day of every other day injections 
  geom_vline(xintercept=30, linetype="dotted", linewidth=1, color="#62748E")+ #increased dose for TZP & SVD
  labs(x="Day",
       y= "ΔBW (%) ",
       title= "Vehicle: ΔBW (%)",
       color="Treatment", fill="Treatment")
...
GLP1_eff <- GLP1_tracker %>%
  filter(ID %in% c(3730, 3731, 3732, 3733, 3735, 3736, 3737, 3738, 3739, 3740, 3741)) %>% #only IDs undergoing injections
  ungroup() %>%
  group_by(ID) %>%
  arrange(DATE) %>%
  mutate(BW_pct_change = 100*((BW - first(BW)) / first(BW)),
         delta_BW_g = BW - lag(BW)) %>%
  ungroup() %>%
  group_by(ID, DATE) %>%
  mutate(Efficiency = delta_BW_g/corrected_intake_kcal)

ggplot(GLP1_eff, aes(x=Treatment_day, y=Efficiency, group=DRUG, fill=DRUG, color=DRUG)) +
  geom_point(stat = "summary", fun = "mean", aes(color=DRUG), size=4) +
  stat_summary(fun = mean,geom = "label",aes(label = round(after_stat(y), 2)), vjust = -3, size = 4, color = "black", # text color
               fill = "white", # label background
               label.padding = unit(0.15, "lines")) + #this seems to make the text boxes smaller but doesn't change text size
  geom_line(stat = "summary", fun = "mean", aes(color=DRUG), linewidth=2) +
  geom_errorbar(stat = "summary", fun.data = mean_se, aes(width=0.08), width=0.45, linewidth=1, color="black") +
  ...
  geom_vline(xintercept=30, linetype="dotted", linewidth=1, color="#62748E")+ #increased dose for TZP & SVD
  labs(x="Day",
       y= "Energy efficiency (delta BW/FI)",
       title= "Daily energy efficiency",
       color="Treatment", fill="Treatment")


#Graph: Look at correlation between change in BW and FI ####
ggplot(GLP1_eff, aes(x=delta_BW_g, y=corrected_intake_kcal, group=DRUG, fill=DRUG, color=DRUG)) +
  geom_point(stat = "summary", fun = "mean", aes(color=DRUG), size=4)
