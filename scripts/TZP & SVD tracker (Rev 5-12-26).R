...
plot_BW_BWloss<-ggplot(GLP1_BWloss_plot, aes(x=Treatment_day, y=BW, group=DRUG, fill=DRUG, color=DRUG)) +
  ...
  theme(legend.position = "right",
        ...
        panel.border = element_blank()) +
  format.plot+
  geom_vline(xintercept=16, linetype="dashed", color="darkgreen")+ #increased doses
  geom_vline(xintercept=20, linetype="dashed", color="darkgray")+ #started doing injections every other day 
  geom_vline(xintercept=23, linetype="dashed", color="darkgray")+ #final day of every other day injections 
  #facet_wrap(~ID)+
  labs(x="Day",
       y= "BW (g)",
       title= "BW (g) during BW loss",
