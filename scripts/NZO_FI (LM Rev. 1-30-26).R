...
#1-28-26: Extended code that I wrote for 3706 at peak obesity to include all IDs at all time points (SABLE)
#Note: keep working on the approach that I started on 1-27-26. Work from 1-26-26 is still incomplete
#1-29-26 Extracted the manual FI information during BW loss for weight cycled mice.
#--> Next step is to do a left join to combine the dfs with summary data for weight cycled at BW loss and mainteanance with the
  #df that I created which contains the summary FI data for all other time point/group combinations
#--> After that, bring in measured EE data to calculate energy balance
...
#Start of 1-28-26 work
#### Extend the code that I used for just 3706 at peak obesity for all mouse IDs at all time points ####
#This creates sable_FI_data (this code is also located earlier in thie script)
sable_FI_data <- sable_dwn %>% 
  filter(COHORT > 2 & COHORT < 6) %>% # Just NZO
  mutate(
    lights = if_else(hr %in% c(20,21,22,23,0,1,2,3,4,5), "Dark", "Light"),
...
summarise (avg_corrected_intake_gr = mean(total_eaten_gr), .groups = "drop") %>%
  filter(!(GROUP== "Weight cycled" & SABLE %in% c("BW loss", "BW maintenance")))

#Average daily FI in grams based on group (as measured by Sable cages)
...
#Need to connect the complete_days to dates...start with one ID = 3708
#Look at FI.csv. Need morning and evening of complete_day=1 and morning and evening of complete_day=2. 
#From FI_data, I can use the Date column and the corrected_intake_gr or corrected_intake_kcal
  #to obtain this information
#For BW loss, this is Sable day 13 and 14 (Sable days 12-15 is the entire duration of the BW loss time point)
...
#Calculate manual FI during BW loss -> avg across middle 2 days of recording and 2 (either complete days 1 and 2 or entire 4 days of Sable recording)
#FI_manual_SABLE has manual FI for all SABLE_DAY_#. Column is called sable_idx
Manual_FI_complete_days <- FI_manual_SABLE %>%
  mutate(ID = factor(ID)) %>%
  mutate ("ID" = as.factor(ID)) %>%
  filter(GROUP == "Weight cycled") %>%
  filter(sable_idx %in% c("SABLE_DAY_13", "SABLE_DAY_14", "SABLE_DAY_17", "SABLE_DAY_18")) %>% # just complete days 1 and 2 from BW loss
  ungroup() %>%
group_by(ID, SABLE, GROUP) %>%
  summarise(avg_corrected_intake_gr = mean(corrected_intake_gr))


#Calculate manual FI during BW loss --> avg across all 4 sable days at BW loss time point
Manual_FI <- FI_manual_SABLE %>%
  filter(GROUP == "Weight cycled") %>%
  filter(SABLE == "BW loss") %>% #includes all 4 Sable days at BW loss time point
  ungroup() %>%
  group_by(ID) %>%
  summarise(BWloss_intake_manual_gr = mean(corrected_intake_gr),
            BWloss_intake_manual_kcal = mean(corrected_intake_kcal))
#### Compare restricted and ad libitum FI while in Sable at the BW loss time point ####
...
#Find the mean of the avg. daily FI for mice in Weight cycled vs control groups measured by cages at each time point,
  #except for BW loss and BW maintenance time weight cycled mice 
Avg_Sable_FI <- sable_FI_minutes %>%
  ungroup() %>%
  group_by(SABLE, GROUP) %>%
  summarise(Avg_GROUP= mean(avg_corrected_intake_gr), .groups="drop")

##### Started here on 1-30-26 ####
#### 
#Combine df1= Manual_FI_complete_days (BW loss and maintenance for weight cycled mice) and
  #df2= sable_FI_minutes (All time points for Control and baseline/peak obesity/BW regain for weight cycled mice)
#Now i have a data frame with average daily FI while in sable for all mice while they were recording in SABLE
  #This excludes mice which had issues with Sable recording (except for at BW loss and maintenance time points for weight cycled mice)
FI_in_Sable <- bind_rows(FI_in_Sable, Manual_FI_complete_days)

#Add column with avg daily kcal (rather than grams) 
...
#Graph FI_in_Sable (can do kcal or grams)
ggplot(FI_in_Sable_gr_kcal, aes(x = SABLE, y = avg_corrected_intake_kcal, fill = GROUP)) +
  ...
  geom_errorbar(
    ...
    width = 0.3) +
  labs(
    title= "Average daily food intake during Sable recording",
    x = "Time point",
