...
All_movement_df <- filter_ped_alm_RQ %>%
  ...
  filter(distance_m >=0) %>%
  summarise(total_distance_m = sum(distance_m), .groups="drop") %>%
  mutate(distance_hr_m = if_else(lights == "off", total_distance_m/10, total_distance_m/14),
    GROUP = case_when(
      ID %in% c(3706, 3707, 3711, 3713, 3716, 3719, 3726) ~ "Control",
      ID %in% c(3708, 3710, 3714, 3720, 3721, 3722, 3727, 3728, 3729) ~ "Weight cycled"),
    DRUG = case_when(
      ...
      ID %in% c(3708, 3710, 3716, 3719, 3721, 3722, 3726, 3729) ~ "RTIOXA_47")) %>%
  mutate(SABLE = factor(SABLE, levels = c("Baseline","Peak obesity","BW loss","BW maintenance","BW regain")))
#Light and dark cycle aren't the same duration, so in order to compare them I need to use meters/hr
#--> this is being done as a correction to allow for direct comparison
#distance_hr_m = if_else(lights == "off", total_distance_m/10, total_distance_m/14)


#Bar plot: AllMeters
ggplot(All_movement_df,
       aes(x = lights, y = distance_hr_m,
  ...
  theme(legend.position = "right",
        ...
        legend.text = element_text(size = 11)) +
  labs(x="DateTime",
       y= "Total locomotion (m/hr)",
       title= "All locomotion (hourly)") +
  facet_wrap(~SABLE)

#Box plot: AllMeters
ggplot(All_movement_df, aes(x = lights, y = distance_hr_m, fill = GROUP)) + 
  geom_boxplot() +
  stat_summary(fun = "mean", geom = "point", shape = 8,
               size = 2, color = "white") +
...
#AllMeters statistical analysis
model_rs <- lmer(distance_hr_m ~ SABLE * GROUP * lights + (1 | ID), data = All_movement_df)
emm <- emmeans(model_rs, ~ SABLE * GROUP * lights)

# Example: compare light vs dark within each group/timepoint
contrast(emm, method = "pairwise", by = c("SABLE", "GROUP"))


#Reorganize data so that it can be graphed as light, dark, and total at each Sable time point
#Also, compare sum of 
