...
ggplot(df4, aes(x = Transition, y = phase_BW_FI, fill = GROUP)) +
...
#Graph ΔBW (g) for Control and Weight cycled mice--> may help explain how weight cycled mice at more but lost weight
    ggplot(df4, aes(x = Transition, y = delta_BW_g, fill = GROUP)) +
      ...
      theme(
        ...
        strip.text = element_text(face = "bold", size = 12)) +
      labs(
        title="Change in body weight during each period of weight cycling",
        x = "Phase of weight cycle",
        y = "ΔBW (g)",
        fill = "Treatment group")
...
#Graph ΔBW (percent) for Control and Weight cycled mice--> may help explain how weight cycled mice at more but lost weight
ggplot(df5, aes(x = Transition, y = delta_BW_perce, fill = GROUP)) +
  ...
  theme(
    ...
    strip.text = element_text(face = "bold", size = 12)) +
  labs(
    title="Percent change in body weight during weight cycling",
    x = "Phase of weight cycle",
    y = "Percent change in BW",
    fill = "Treatment group")
...
#Graph daily FI over the course of BW loss (look at daily fluctuation in each group)
  ggplot(Explore_flux, aes(x = DATE, y = corrected_intake_kcal, fill = GROUP)) +
  geom_bar(stat = "summary", 
           fun = "mean", 
           position = position_dodge(width = 0.9)) +
  geom_errorbar(stat = "summary", 
  ...
  theme(
    ...
    strip.text = element_text(face = "bold", size = 12)) +
  labs(
    title="Daily food intake (kcal) during weight loss",
    x = "Date",
  ...
  #facet_wrap(~ID)
#--------------------------------- #
  #Not part of an objective...
# FI graph (made this because I wanted to quickly see if control mice had decreased FI during injections)
    ggplot(df4, aes(x = Transition, y = Daily_kcal, fill = GROUP)) +
    geom_bar(stat = "summary", 
             fun = "mean", 
             position = position_dodge(width = 0.9)) +
    geom_errorbar(stat = "summary", 
