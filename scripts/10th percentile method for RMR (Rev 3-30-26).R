...
# TEMPORARY on 3-30-26 ####
#Is 3707 an outlier? Try removing it from EE calculations and see if things are still significant
Daily_EE_no3707 <- Daily_EE %>%
  ungroup() %>%
  group_by(ID, SABLE, GROUP) %>%
  filter(!ID==3707)
...
#Can I modify this to get hourly EE during light and dark period? ####
#Calculate average EE (in units of kcal/hr) during each photo period...perhaps
#a better approach would be to sum during light and during dark and divide the sum by the number of hours in the period

Photo_mean_EE <- Minute_sum_EE_hr %>%
  mutate(lights= if_else(hr %in% c(20,21,22,23,0,1,2,3,4,5), "off", "on")) %>% #add photo-period
  ungroup() %>%
  group_by(ID, SABLE, GROUP, lights) %>%
  summarise(TEE_kcal_hr = mean(TEE_kcal), #average hrly TEE during each photo period for each mouse
            NEAT_kcal_hr = mean(NEAT_kcal),
            RMR_kcal_hr = mean(RMR_kcal))

#---Graph raw TEE (average kcal/hr) across the light and dark photo period ####
ggplot(Photo_mean_EE, aes(x = lights, y = TEE_kcal_hr, fill = GROUP)) +
  geom_bar(stat = "summary", 
           fun = "mean", 
           position = position_dodge(width = 0.9)) +
  geom_errorbar(stat = "summary", 
  ...
  scale_fill_manual(values = custom_colors) +
  labs(
    title="TEE by photo period (average kcal/hour)--Raw",
    x = "Photo period",
    y = "TEE (average kcal/hour)",
    fill = "Treatment group")

#Could use ANCOVA for a within time point assesment of whether there is a 
#treatment group based difference during the light cycle or during the dark cycle
#ANCOVA: outcome is EE, covariate
#actually maybe repeated measures anova would be better. All mice underwent 
#both the light and dark conditions
