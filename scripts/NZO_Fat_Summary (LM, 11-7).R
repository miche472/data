#Goal of script: Graph and do stats for NZO Fat mass

# Revised: 11-7-25

#Need to run script to make sable_TEE_RMR_adj to use as source data for fat mass (Fat)
#Located at the bottom of this script if it is not already in the Environment

# Fat mass: ANOVA, T-test, linear mixed model and emmeans, bar graph
# Use sable_tee_adj_RMR (uses echoMRI_data lean mass data and only the mice with Sable measures)
...
#For consistency between y-axis of Fat mass and Fat free mass graphs:
  #scatterplots range from 
  #bar plots range from 0-35
####----- Graphs of fat mass at the 5 time points (n=16 NZO mice) -----####
...
#####----- Barplot - NZO fat mass (measured values from echoMRI_data/sable_TEE_adj_RMR) ####
Fat_barplot_measured <- sable_TEE_adj_RMR %>%
  ggplot(aes(x = SABLE, y = Fat, fill = GROUP)) + 
  ...
  scaleColor +
  theme_minimal() +
  labs(title = "NZO fat mass", 
       y = "Fat mass (grams)", 
       x= "Time point", 
       color = "Treatment group",
       fill = "Treatment group") +
  #scale_y_continuous( # set y-axis breaks every 10 grams
  #breaks = seq(0, max(sable_TEE_adj_RMR$Fat, na.rm = TRUE), by = 10)) +
  format.plot +
...
Fat_barplot_measured

##### Scatterplot - NZO fat mass (measured values from echoMRI_data/sable_TEE_adj_RMR)####

Fat_scatterplot_measured <- sable_TEE_adj_RMR %>%
  ggplot(aes(x = SABLE, y = Fat, color = GROUP)) + 
  #geom_point(position = position_jitterdodge(jitter.width = 0.15, dodge.width = 0.6), alpha = 0.7,size = 2) +
  ...
  labs(title = "NZO fat mass", 
    ...
    x = "Time point", 
    color = "Treatment group") +
  #scale_y_continuous(breaks = seq(0, max(sable_TEE_adj_RMR$Fat, na.rm = TRUE), by = 10)) +
  format.plot +
  theme(legend.position = "top", 
        axis.ticks.y = element_line(),
        plot.title = element_text(hjust = 0.5, face = "bold"), 
...
ttest_results_Fat <- sable_TEE_adj_RMR %>%
  group_by(SABLE) %>%
...
anova_results_Fat <- sable_TEE_adj_RMR %>%
  ungroup() %>%
  group_by(GROUP) %>%
  anova_test(dv = Fat, wid = ID, within = SABLE)
anova_results_Fat

#Post-hoc test for ANOVA
pairwise_results_Fat <- sable_TEE_adj_RMR %>%
  group_by(GROUP) %>%
  pairwise_t_test(Fat ~ SABLE, paired = TRUE, p.adjust.method = "bonferroni") %>%
  filter(p.adj.signif <= 0.05)
pairwise_results_Fat

####----- Linear mixed model, emmeans, contrasts -----####
# Build linear mixed model
model_Fat <- lmer(Fat ~ SABLE * GROUP + (1 | ID), data = sable_TEE_adj_RMR)
summary(model_Fat)
...
contrasts_Fat_df <- as.data.frame(contrasts_by_group_Fat)
contrasts_Fat_df
#Filter for significant contrasts
Sig_contrasts_Fat <- contrasts_Fat_df %>%
  filter(p.value <=0.05)
Sig_contrasts_Fat
...
# Convert to a data frame
contrasts_SABLE_Fat_df <- as.data.frame(contrasts_by_SABLE_Fat)
contrasts_SABLE_Fat_df
...
barplot_emm_Fat <- emm_Fat_df %>%
  ...
  theme_minimal() +
  labs(title = "NZO Fat mass (emmeans)",
    y = "Fat mass (grams)",
    x = "Time point",
    fill = "Treatment group") +
  ylim(0, 30) +
