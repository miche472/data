...
#Look at energy expenditure for observations 24hrs after injection ####
OXA_Minute_sum_EE_hr <- OXA_loc_EE %>%
  ...
  ungroup() %>%
  group_by(DOSE, ID, hrs_post) %>% # group by lights to retain photoperiod in df for later use
  summarise(
    n_obs = n(),
