...
Manual_FI_complete_days <- FI_wc %>%
  filter(GROUP == "Weight cycled") %>%
  filter(sable_idx %in% c("SABLE_DAY_13", "SABLE_DAY_14")) %>% # just complete days 1 and 2 from BW loss
  ungroup() %>%
  group_by(ID) %>%
...
Avg_Sable_FI <- sable_FI_minutes %>%
  ungroup() %>%
  group_by(SABLE, GROUP) %>%
  summarise(Avg_GROUP= mean(Avg_two_days_gr), .groups="drop")

##### Left off here on 1-29-26: pick up here on 1-__-26 ####
#### join df with manual food measurements for restricted mice during BW loss/maintenance with the df that has FI 
  #directly measured by the Sable cages at each time point (except the BW loss/mainteance time point)
