...
#Script revised: 3-13-26
#This script was revised from: "New method energy balance (rev 3-6-26) version B.R"
...
#Pretty good method for calculating TEE --> graph looks good, but stats aren't ideal
#Used t test to see if there is a significant difference in raw TEE for control and weight 
    #cycled mice at BW regain. p value was about 0.1

#Objectives ####
#Objective 1: calculate TEE using zt time, is_complete_day, and complete days.  
    #Then use an averaging method (hourly average, summed) to extrapolate TEE.
#Objective 2:Graph of EE vs Mass (tells you if you should use ancova or generalized linear model)
#Objective 3: MLR for TEE adjusted for lean or BW
#Objective 4: Break TEE down by light and dark photo period
#Objective 5a: Use sable recording to measure FI during iCAL
#Objective 5b: Use manual data to measure FI during iCAL
#Objective 6: Energy balance (calculation, MLR, graphed estiamtes). Includes method using sable recorded 
    #and manual INTAKE
...
#method for complete day was Carolina's method with zt time, is_complete_day, complete_days
mod_TEE_day_regain <- mod_TEE_day %>% 
  ...
  res
  #For student t test, p=0.1063 for the comparison of control and weight cycled mice at BW regain
#--> Fail to reject the null hypothesis that the difference in TEE between groups is 0 after BW regain
...
#--
#Objective 2:Graph of EE vs Mass (tells you if you should use ANCOVA or generalized linear model) ####
#This idea came from the Sable seminar by Banks, the first author of the consensus paper for iCAL data
#For just the BW regain time point:
Echo_mod_TEE_day_regain <-Echo_mod_TEE_day %>%
  filter(SABLE=="BW regain")

ggplot(Echo_mod_TEE_day_regain, aes(x = Weight, y = sum_avg_TEE_hr, color = GROUP)) +
  geom_point() +
  ...
  format.plot +
  labs(
    title="BW regain: TEE vs BW for control and weight cycled",
    x = "Body weight (g)",
    y = "TEE (kcal/day)",
    fill = "Treatment group")
#Lines for control and weight cycled mice are not parallel, so an ANCOVA isn't possible
#According to Banks et al. 2025, you must instead use a generalized linear model 
...
# MLR for TEE adjusted for BW ####
#MLR for TEE adjusted for BW
#Build multiple linear regression model for TEE adjusted for BW (use df = mod_TEE_day)
model_mod_TEE_day_weight <- lmer(sum_avg_TEE_hr ~ SABLE * GROUP + Weight + (1 | ID), data = Echo_mod_TEE_day)
summary(model_mod_TEE_day_weight)

# Calculate estimated marginal means 
emm_mod_TEE_day_weight <- emmeans(model_mod_TEE_day_weight, ~ SABLE * GROUP, cov.reduce = mean)
emm_mod_TEE_day_weight_df <- as.data.frame(emm_mod_TEE_day_weight)

# Pairwise contrasts within each GROUP
contrasts_by_group_mod_TEE_day_weight <- contrast(emm_mod_TEE_day_weight, method = "pairwise", by = "GROUP")
contrasts_by_group_mod_TEE_day_weight_df <- as.data.frame(contrasts_by_group_mod_TEE_day_weight)

# Pairwise contrasts within each SABLE (time point)
contrasts_by_SABLE_mod_TEE_day_weight <- contrast(emm_mod_TEE_day_weight, method = "pairwise", by = "SABLE")
contrasts_SABLE_mod_TEE_day_weight_df <- as.data.frame(contrasts_by_SABLE_mod_TEE_day_weight)

#Conclusions from MLR for TEE adjusted for body weight (BW) (df=Echo_mod_TEE_day)
#For time point comparisons the only significant difference is at BW maintenance --> unexpected

#Graph MLR for TEE adjusted for body weight --> Bar plot
barplot_emm_mod_TEE_day_weight <- emm_mod_TEE_day_weight_df %>%
  ggplot(aes(x = SABLE, y = emmean, fill = GROUP)) +
  # mean bars
  geom_col(position = position_dodge(width = 0.8),
           color = "black", width = 0.7, alpha = 0.7) +
  ...
  labs(
    title = "Total energy expenditure (kcal/day)",
    y = "TEE (kcal/day) adjusted for body weight",
    x = "Time point",
    fill = "Treatment group")
barplot_emm_mod_TEE_day_weight
...
#Step 3b: graph TEE during lights on and off (kcal/photo period)
#Dark and light period are not of equal duration, so this method is misleading. See step 3c
...
#Graph 3c: Average EE during light and dark period (kcal/hr)
...
#---
#Objective 5a: use sable recording to measure FI during iCAL ####
#Below is an approach using Sable measurements --> too many measurements seemed to be incorrect. 
    #I don't know that Sable measurements of FI are reliable
...
mod_sable_FI <- sable_FI_modCS %>%
  ...
  #filter(!(ID==3707)) %>%
  filter(ID==3706, SABLE=="BW regain") %>%
  ...
  #check how many values are between 0 to 0.1g per minute
  filter(intake>0)
...
#Calculate manual FI just for weight cycled mice at BW loss & maintenance (i.e. during calorie restriction)
...
#To combine the FI recorded by Sable with the manual FI for weight cycled mice during caloric restriction
#Note: I probably won't use this since the Sable FI doesn't seem reliable 
...
#--
#Objective 6: Energy balance (calculation, MLR, graphed estiamtes) ####

#---Energy balance (original/less accurate method) --> uses mix of manual FI and sable recorded FI
...
#---Energy balance (alternative/more accurate method)--> uses manual FI
#EE is calculated in this script. For EE, use df= mod_TEE_day where daily EE is sum_avg_TEE_hr
...
#The section below is not complete ####
