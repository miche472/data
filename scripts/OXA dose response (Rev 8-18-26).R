...
BW_COHORT19_Baseline <- read_csv("~/Documents/GitHub/data/data/BW.csv") %>%
  filter(COHORT == 19) %>%
  filter(ID %in% c(3731,3732,3733,3735,3737,3738,3739,3740,3741))%>%
  mutate(PERIOD = case_when(ID %in% c(3731,3732,3733,3735,3737,3739,3740,3741) & DATE =="2026-07-17" ~ "0",
                             ID==3738 & DATE =="2026-08-03" ~ "0"),
         PERIOD=as.integer(PERIOD),
         ID=as.factor(ID)) %>%
  drop_na(PERIOD)
...
Baseline_sum_EE_hr_bins <- Baseline_loc_EE_60min %>%
  ...
  ungroup() %>%
    left_join(
    BW_COHORT19_Baseline %>% 
      select(ID, BW, PERIOD), 
    by = c("ID")) %>%
  mutate(DOSE= case_when(PERIOD =="0"~"Baseline"),
         DOSE = as.factor(DOSE))

### Calculate sum of EE from 5-7pm (hrs 0 to 2), 7-9pm (hrs 2-4), & 5-9pm (hrs 0-4) ####
Baseline_sum_EE_summary <- Baseline_sum_EE_hr_bins %>%
  group_by(ID) %>%
  summarise(
...
### Calculate avg kcal/hr EE from 5-7pm (hrs 0 to 2), 7-9pm (hrs 2-4), & 5-9pm (hrs 0-4) ####
Baseline_avg_EE_summary <- Baseline_loc_EE_60min %>%
  group_by(ID) %>%
  summarise(
    Avg_EE_0_2hr = mean(Kcal_Hr[recording_bin >= 0 & recording_bin < 2], na.rm = TRUE),
    Avg_EE_2_4hr = mean(Kcal_Hr[recording_bin >= 2 & recording_bin < 4], na.rm = TRUE),
    Avg_EE_0_4hr = mean(Kcal_Hr[recording_bin >= 0 & recording_bin < 4], na.rm = TRUE),
    .groups = "drop") %>%
  left_join(
    BW_COHORT19_Baseline %>% 
      select(ID, BW, PERIOD), 
...
#---
#---
# Compile Baseline and OXA data for EE (sum and average) ####
#Sum EE (summation of each minute of data): df=Baseline_sum_EE_hr_bins (EE expressed as sum_EE_60min) 
            #and df=OXA_loc_EE_60min (EE expressed as TEE_kcal)
#First, revise OXA_loc_EE_60min to include BW and correctly named variables ->
#rename the OXA df to "OXA_sum_EE_hr_bins"
OXA_sum_EE_hr_bins <- OXA_loc_EE_60min %>%
  group_by(ID, PERIOD, recording_bin, DOSE) %>%
  mutate(sum_EE_60min = TEE_kcal) %>%
    left_join(
    BW_COHORT19 %>% 
      select(ID, BW, PERIOD), 
    by = c("ID", "PERIOD"))

#Combine summed TEE dfs for baseline and OXA doses
Baseline_OXA_sum_EE_hr_bins <- bind_rows(
  OXA_sum_EE_hr_bins,
  Baseline_sum_EE_hr_bins)

### Calculate sum of EE from 5-7pm (hrs 0 to 2), 7-9pm (hrs 2-4), 5-9pm (hrs 0-4), & 24hrs ####
Summary_EE_sum <- Baseline_OXA_sum_EE_hr_bins %>%
  group_by(ID, DOSE) %>%
  summarise(
    sum_EE_0_2hr = sum(sum_EE_60min[recording_bin >= 0 & recording_bin < 2], na.rm = TRUE),
    sum_EE_2_4hr = sum(sum_EE_60min[recording_bin >= 2 & recording_bin < 4], na.rm = TRUE),
...
#Avg EE (avg kcal/hr recorded): Baseline_avg_EE_hr_bins (EE expressed as avg_EE_60min) 
            #and df=OXA_loc_EE_60min_avg (EE expressed as avg_kcal_per_hr)

OXA_loc_EE_to_combine <- OXA_loc_EE %>%
  group_by(ID, PERIOD, DOSE, DateTime) %>% 
  arrange(DateTime) %>%
  mutate(recording_bin = floor(minutes_post_recording / 60)) %>%
  ungroup() %>%
