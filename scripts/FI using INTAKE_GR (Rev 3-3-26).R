...
#--------------------------#
#--------------------------#
#First you must create FI_BW_joined (code for this is located at bottom of this script)
#Next, create a df that has food intake calculated from INTAKE_GR (rather than intake_corrected)
df_hi <- FI_BW_joined %>%
      ungroup() %>%
      group_by(ID) %>% 
      arrange(DATE) %>% 
...
#Graph TOTAL FI for Control and Weight cycled mice during each Transition ####
    #Total FI during each phase, by Treatment group -> use INTAKE_GR method
    ggplot(df42, aes(x = Transition, y = INTAKE_kcal, fill = GROUP)) +
         ...
         theme(
           ...
           strip.text = element_text(face = "bold", size = 12)) +
         labs(
           title="Food intake during weight cycling (INTAKE_GR method)",
           x = "Phase of weight cycle",
           y = "Intake (kcal)",
           fill = "Treatment group")
...
#Graph Daily FOOD INTAKE for Control and Weight cycled mice during each Transition ####
  ...
  #Daily FI allows for "correction" for different durations of time spent in each phase 
       ggplot(df42, aes(x = Transition, y = Daily_INTAKE_kcal, fill = GROUP)) +
         ...
         theme(
           ...
           strip.text = element_text(face = "bold", size = 12)) +
         labs(
           title="Average daily food intake during weight cycling",
           x = "Phase of weight cycle",
           y = "Daily food intake (kcal)",
           fill = "Treatment group")
       
#Multiple linear regression to determine if average daily FI is significantly different
        #between time points and control/weight cycled mice
       
    #Build multiple linear regression model for daily food intake (use df = df42)
    model_Daily_INTAKE_kcal <- lmer(Daily_INTAKE_kcal ~ Transition * GROUP + (1 | ID), data = df42)
    summary(model_Daily_INTAKE_kcal)
       
    # Calculate estimated marginal means 
    emm_Daily_INTAKE_kcal <- emmeans(model_Daily_INTAKE_kcal, ~ Transition * GROUP, cov.reduce = mean)
    emm_Daily_INTAKE_kcal_df <- as.data.frame(emm_Daily_INTAKE_kcal)
       
    # Pairwise contrasts within each GROUP
    contrasts_by_group_Daily_INTAKE_kcal <- contrast(emm_Daily_INTAKE_kcal, method = "pairwise", by = "GROUP")
    contrasts_by_group_Daily_INTAKE_kcal_df <- as.data.frame(contrasts_by_group_Daily_INTAKE_kcal)
       
    # Pairwise contrasts within each SABLE (time point)
    contrasts_by_Transition_Daily_INTAKE_kcal <- contrast(emm_Daily_INTAKE_kcal, method = "pairwise", by = "Transition")
    contrasts_Transition_Daily_INTAKE_kcal_df <- as.data.frame(contrasts_by_Transition_Daily_INTAKE_kcal)
       
#Conclusions:RMR/TEE ####
      #Within GROUP contrasts: during obesity development and during BW regain, weight cycled mice had 
      #the same daily food intake (p=0.142). Lots of good differences -> keep exploring
      #Within transition periods: Peak obesity to BW loss and BW loss to maintenance is sig diff between GROUPs
       
#Bar plot - Graph predicted daily intake during each transition period (kcal/day)
       barplot_emm_Daily_INTAKE_kcal <- emm_Daily_INTAKE_kcal_df %>%
         ...
         theme_minimal() +
         labs(
           title = "Predicted average daily food intake (kcal)",
           y = "Daily food intake (kcal)",
           x = "Transition period",
           fill = "Treatment group") +
         format.plot +
