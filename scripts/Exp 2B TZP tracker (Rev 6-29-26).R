...
Exp2B_tracker <- BW_FI_19 %>%
  ...
  mutate( 
    ...
    STAGE = case_when(ID %in% c(3744, 3745, 3748) 
                      & DATE< "2026-06-21" ~ "BW gain",
                      ID %in% c(3744, 3745, 3748) 
                      & DATE> "2026-06-20" ~ "BW loss",
                      
                      ID %in% c(3746, 3752) 
                      & DATE< "2026-06-24" ~ "BW gain",
                      ID %in% c(3746, 3752) 
                      & DATE>= "2026-06-23" ~ "BW loss"),
...
ggplot(Exp2B_BWloss_plot, aes(x = Treatment_day, y = BW, fill = DRUG, color=DRUG)) +
  ...
  format.plot+
  #facet_wrap(~ID)+
 geom_vline(xintercept=7, linetype="dotted", linewidth=1, color="#62748E")+ #increased dose 10->15
  #geom_vline(xintercept=14, linetype="dotted", linewidth=1, color="#62748E")+ #Stopped injections
  labs(x="Day",
       y= "Body weight (g)",
