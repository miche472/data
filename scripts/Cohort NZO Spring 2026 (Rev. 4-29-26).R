...
#Try doing a different assignment method ####
...
####ANOVAs for AI sorted ####
#One way ANOVA for AI sorted: same mean adiposity index for all three groups?
ANOVA_AI_sorted_checkAI_new <- aov(adiposity_index ~ treatment, data = echoMRI_data_Cnew)
summary(ANOVA_AI_sorted_checkAI_new)

#Perform Tukey's Test for multiple comparisons
TukeyHSD(ANOVA_AI_sorted_checkAI_new, conf.level=.95) 

#Shapiro Wilk test to check normality of residuals
shapiro.test(residuals(ANOVA_AI_sorted_checkAI_new))
#Shapiro Wilk test tests the null hypothesis that the samples come from a normal distribution
#the p value is 0.553 suggesting that they do come from a normal distribution

#Check for equal variance
#Create box plots that show distribution of adiposity index for each group
boxplot(adiposity_index ~ treatment, xlab='Treatment group', ylab='Adiposity index (fat/lean)', data=echoMRI_data_Cnew)
#Conduct Bartlett's test for variance -> tests the null hypothesis that the samples have 
#Check for equal variances
bartlett.test(adiposity_index ~ treatment, data=echoMRI_data_Cnew)
#p value is 0.4651 --> samples have equal variance
