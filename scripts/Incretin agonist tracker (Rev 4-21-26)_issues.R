...
# MUST check this before saving ! ####
# --> Does the solution that I used lose and/or duplicate any data?

#Possible solution: 
#drop dates when there is no value for INTAKE_GR
#then use lag function to calculate delta_measurement_FI
#then do left join of the df with accurate delta_measurement_FI with 


# output food-intake file
FI_intermediate <- cohort_open_files %>% 
  select(ID, DIET_FORMULA, INTAKE_GR, DATE,COMMENTS) %>% 
  group_by(ID) %>% 
  arrange(DATE, .by_group = TRUE) %>% 
  drop_na(INTAKE_GR) %>% #possible solution to get accurate values for delta days for FI drop dates when there is no INTAKE_GR
  mutate(
    delta_measurement_FI = DATE - lag(DATE)) %>% 
  drop_na(delta_measurement_FI) %>% 
  mutate(
    corrected_intake_gr = INTAKE_GR / as.numeric(delta_measurement_FI) #DAILY INTAKE
  ) %>% 
  left_join(., food_desc, by = "DIET_FORMULA") %>% 
  mutate(
    corrected_intake_kcal = corrected_intake_gr * KCAL_G) 
  
  FI <- cohort_open_files %>% #Join FI and BW
  left_join(., FI_intermediate, by = c("ID", "DATE"))
  
  #Could left join FI with cohort_open_files using ID and DATE...would this restore the lines that were dropped due to lack of FI data?
  left_join(., metadata, by = "ID")

#END of section that must be checked before saving ####
  
  
#Alternative approach for delta_days
  #Could also change the mutate statement to be if_else...if INTAKE_GR >=0 then delta_measurement=DATE-lag(DATE)
  #otherwise delta_measurement = NA...this approach would avoid dropping rows and re-attaching 
FI_alternative <- cohort_open_files %>% 
    select(ID, DIET_FORMULA, INTAKE_GR, DATE,COMMENTS) %>% 
    group_by(ID) %>% 
    arrange(DATE, .by_group = TRUE) %>% 
    mutate(delta_alt = if_else(INTAKE_GR>0, DATE - lag(DATE), NA))
           
     

# DELETE this after referencing 
...
ggplot(GLP1_BWloss_plot, aes(x = Inject_day, y = BW, fill = DRUG, color=DRUG)) +
  geom_line(stat = "summary", 
           fun = "mean") +
  geom_point() +
  scale_color_manual(values = custom_colors_GLPB) +
  theme_bw(base_size = 14) +
  theme(legend.position = "right",
