...
#cummulative EE using the 10th percentil method for RMR 
...
##---Graph EE by photo period (RAW EE) ####
## Graph--TEE in Light period (RAW) ####
Light_Scaled_Photo_EE <- Scaled_Photo_EE %>%
  filter(lights=="on")

ggplot(Light_Scaled_Photo_EE, aes(x = SABLE, y = scaled_TEE, fill = GROUP)) +
  geom_bar(stat = "summary", 
           fun = "mean", 
           position = position_dodge(width = 0.9)) +
  geom_point(position = position_jitterdodge(dodge.width = 0.8, jitter.width = 0.2),
  ...
  format.plot+
  scale_fill_manual(values = custom_colors) +
  labs(title="TEE in light period (kcal)",
    x = "Time point",
    y = "TEE (cumulative kcal in light period)",
    fill = "Treatment group")

##Graph--TEE in Dark period (RAW) ####
Dark_Scaled_Photo_EE <- Scaled_Photo_EE %>%
  filter(lights=="off")

ggplot(Dark_Scaled_Photo_EE, aes(x = SABLE, y = scaled_TEE, fill = GROUP)) +
  geom_bar(stat = "summary", 
           fun = "mean", 
           position = position_dodge(width = 0.9)) +
  geom_point(position = position_jitterdodge(dodge.width = 0.8, jitter.width = 0.2),
  ...
  scale_fill_manual(values = custom_colors) +
  labs(
    title="TEE in dark period (kcal)",
    x = "Time point",
    y = "TEE (cumulative kcal in dark period)",
    fill = "Treatment group")
