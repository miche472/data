...
#--> Fail to reject the null hypothesis that the difference in TEE between these two groups is 0 after BW regain
...
#--
#Graph of EE vs Mass (tells you if you should use ancova or generalized linear model
...
barplot_emm_mod_TEE_day_weight <- emm_mod_TEE_day_weight_df %>%
  ...
  theme_minimal() +
  labs(
    title = "Daily TEE adjusted for body weight (df=Echo_mod_TEE_day)",
    y = "Daily TEE adjusted for body weight",
    x = "Time point",
    fill = "Treatment group") +
  format.plot +
  theme(legend.position = "top",
...
# end MLR for TEE adjusted for BW ####
