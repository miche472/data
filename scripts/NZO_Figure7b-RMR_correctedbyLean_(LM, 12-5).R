#NZO_Figure7b-RMR_correctedbyLean_(LM, 12-5)
#Script revised: 12-5-25. Revised version of -> NZO_Figure7b-RMR_correctedbyLean (LM, 10-30)
  #Revised to filtered out all of the mice that needed to be removed based on sable cage malfunction
  #during each step that created data frames
  #I ran this after re-running dat_proc.R after Carolina fixed the issue with the META.csv
  #the issue was that there was an incorrect date for one of the SABLE_DAY_ for a few NZO mice
...
contrasts_RMR_lean_df <- as.data.frame(contrasts_by_group_RMR_lean)

#Filter for significant contrasts
Sig_contrasts_RMR_lean <- contrasts_RMR_lean_df %>%
  filter(p.value <=0.05)
Sig_contrasts_RMR_lean
...
ggplot() +
  ...
  geom_errorbar(data = emm_RMR_lean_df,
  ...
  labs(y = "Adjusted RMR (kcal/day)", x = "Time point",
       ...
       title = "Resting metabolic rate adjusted for FFM") +
  format.plot +
...
#-----------------------------------------
#### Build linear mixed model for RMR (adjusted for BW) ####
...
# Pairwise contrasts within each GROUP
contrasts_by_group_RMR_Weight <- contrast(emm_RMR_Weight, method = "pairwise", by = "GROUP")
# Convert to a data frame
contrasts_RMR_Weight_df <- as.data.frame(contrasts_by_group_RMR_Weight)

#Filter for significant contrasts
Sig_contrast_RMR_Weight <- contrasts_RMR_Weight_df %>%
  filter(p.value <=0.05)
Sig_contrast_RMR_Weight

# Pairwise contrasts within each SABLE (time point)
contrasts_by_SABLE_RMR_Weight <- contrast(emm_RMR_Weight, method = "pairwise", by = "SABLE")
# Convert to a data frame
contrasts_SABLE_RMR_Weight_df <- as.data.frame(contrasts_by_SABLE_RMR_Weight)

#### Plots (RMR and BW) ####

#### Scatter plot: predicted NZO RMR adjusted for body weight (Weight) ####
#I commented out geom_jitter to remove the measured values from the graph 
#(since y-axis title says "adjusted" meaning "predicted")
ggplot() +
  #geom_jitter(data = sable_TEE_adj_RMR, 
  #aes(x = SABLE, y = RMR_kcal_day, color = GROUP),
  #width = 0.2, alpha = 0.4, size = 2) +
  ...
  geom_line(data = emm_RMR_Weight_df,
            ...
            position = position_dodge(0.2), linewidth = 1.5) + 
  scale_color_manual(values=c('#FAAC41','#5392DB')) +
  geom_errorbar(data = emm_RMR_Weight_df,
                aes(x = SABLE, ymin = emmean - SE, ymax = emmean + SE, color = GROUP),
                width = 0.15, position = position_dodge(0.2)) +
  theme_minimal(base_size = 14) +
  labs(y = "Adjusted RMR (kcal/day)", x = "Time point",
       color = "Treatment group",
       title = "Resting metabolic rate adjusted for BW") +
  format.plot +
  theme(legend.position = "top", 
        plot.title = element_text(hjust=0.5), 
        axis.text = element_text(color="black", size=12),
...
#### Bar plot - Graph predicted RMR adjusted for Weight ####
# Define custom colors
custom_colors <- c(
  "Control" = "#FAAC41",              
  "Weight cycled" = "#3498DB")

barplot_emm_RMR_Weight <- emm_RMR_Weight_df %>%
  ...
  theme(
    ...
    plot.title = element_text(hjust = 0.5),
    axis.text.x = element_text(angle = 45, hjust = 1))
barplot_emm_RMR_Weight


#### Bar plot organized to have ad lib values adjacent and restricted values adjacent ####
#Aesthetically this is a work in progress (decide how to color)

custom_colors2 <- c(
  "Baseline" = "red",
  "Peak obesity" = "orange",
  "BW loss" = "yellow",
  "BW maintenance" = "green",
  "BW regain" = "blue")

barplot_emm_RMR_Weight_2 <- emm_RMR_Weight_df %>%
  ggplot(aes(x = GROUP, y = emmean, fill = SABLE)) +
  # mean bars
  geom_col(position = position_dodge(width = 0.8),
           color = "black", width = 0.7, alpha = 0.7) +
  # error bars using SE
  ...
  theme_minimal() +
  labs(
    title = "Resting metabolic rate (RMR) adj. for bodyweight",
    y = "Adjusted RMR (kcal/day)",
    x = "Time point",
    fill = "Treatment group") +
  format.plot +
  theme(
    ...
    plot.title = element_text(hjust = 0.5),
    axis.text.x = element_text(angle = 45, hjust = 1))
barplot_emm_RMR_Weight_2

#-------------------------------------------------------------------
#### I used the following chunks of code when I was figuring out/learning how 
#to identify the 30min period with the lowest TEE ####
