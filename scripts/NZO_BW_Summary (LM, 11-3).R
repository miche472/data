#Goal of script: Graph and do stats for NZO BW

# Revised: 11-11-25
# Use this script as a template for lean, fat, & adiposity index

#Need to run script to make sable_TEE_RMR_adj to use as source data for BW etc.
  #Located at the bottom of this script if it is not already in the Environment

# BW: ANOVA, T-test, linear mixed model and emmeans, bar graph
# Use sable_tee_adj_RMR (uses echoMRI_data BW data and only the mice with Sable measures)
...
####----- Graphs of BW at the 5 time points (n=16 NZO mice) -----####
...
#####----- Barplot - NZO body weight (measured values from echoMRI_data/sable_TEE_adj_RMR) ####
BW_barplot_measured <- sable_TEE_adj_RMR %>%
  ggplot(aes(x = SABLE, y = Weight, fill = GROUP)) + 
  ...
  scaleColor +
  theme_minimal() +
  labs(title = "NZO body weight", 
       y = "Body weight (grams)", 
       x= "Time point", 
       color = "Treatment group",
       fill = "Treatment group") +
  scale_y_continuous( # set y-axis breaks every 10 grams
      breaks = seq(0, max(sable_TEE_adj_RMR$Weight, na.rm = TRUE), by = 10)) +
  format.plot +
  theme(
    legend.position = "top",
...
##### Scatterplot -NZO body weight (measured values from echoMRI_data/sable_TEE_adj_RMR)####

BW_scatterplot_measured <- sable_TEE_adj_RMR %>%
  ggplot(aes(x = SABLE, y = Weight, color = GROUP)) + 
  #geom_point(
...
#T-test: pairwise comparison between GROUP (ad lib and restricted) at each STATUS
    ttest_results_BW <- sable_TEE_adj_RMR %>%
      group_by(SABLE) %>%
      t_test(Weight ~ GROUP, var.equal = TRUE) %>%   # or var.equal = FALSE if not assumed
...
# Perform repeated-measures ANOVA separately for each GROUP
    anova_results_BW <- sable_TEE_adj_RMR %>%
      ungroup() %>%
      group_by(GROUP) %>%
      anova_test(dv = Weight, wid = ID, within = SABLE)
    anova_results_BW

#Post-hoc test for ANOVA
    pairwise_results_BW <- sable_TEE_adj_RMR %>%
      group_by(GROUP) %>%
      pairwise_t_test(Weight ~ SABLE, paired = TRUE, p.adjust.method = "bonferroni") %>%
      filter(p.adj.signif <= 0.05)
    pairwise_results_BW

####----- Linear mixed model, emmeans, contrasts -----####
# Build linear mixed model
    model_Weight <- lmer(Weight ~ SABLE * GROUP + (1 | ID), data = sable_TEE_adj_RMR)
...
# Calculate estimated marginal means (emmeans) 
    emm_Weight <- emmeans(model_Weight, ~ SABLE * GROUP, cov.reduce = mean)
    emm_Weight_df <- as.data.frame(emm_Weight)

# Pairwise contrasts within each GROUP
    contrasts_by_group_Weight <- contrast(emm_Weight, method = "pairwise", by = "GROUP")
    # Convert to a data frame
    contrasts_Weight_df <- as.data.frame(contrasts_by_group_Weight)
    contrasts_Weight_df
    #Filter for significant contrasts
    Sig_contrasts_Weight <- contrasts_Weight_df %>%
      filter(p.value <=0.05)
    Sig_contrasts_Weight

# Pairwise contrasts within each SABLE (time point)
    contrasts_by_SABLE_Weight <- contrast(emm_Weight, method = "pairwise", by = "SABLE")
    # Convert to a data frame
    contrasts_SABLE_Weight_df <- as.data.frame(contrasts_by_SABLE_Weight)
    contrasts_SABLE_Weight_df
...
# Define custom colors
  ...
  barplot_emm_Weight <- emm_Weight_df %>%
...
ggplot() +
      ...
      labs(y = "Body weight (g)", x = "Time point",
           color = "Treatment group",
           title = "NZO body weight (emmeans)") +
      format.plot +
