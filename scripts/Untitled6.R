...
#### LM: from chat gpt --> want to standardize by the number of days that each mouse spent in each transition phase ####
# Summarize cumulative FI per ID and STATUS ----
FI_stage_summary <- FI_data %>%
  group_by(ID, STRAIN, GROUP, DRUG, STATUS) %>%
  summarise(
    ...
    days_in_phase = as.numeric(end_date - start_date) + 1,
    .groups = "drop"
  ) %>%
  # Reshape into wide format: one row per ID, columns = each STATUS
  pivot_wider(
    names_from = STATUS,
    values_from = c(FI_cum_end, days_in_phase)
  ) %>%
  # Calculate kcal consumed between stages and days per interval
  mutate(
    kcal_baseline_to_peak = `FI_cum_end_peak obesity` - FI_cum_end_baseline,
    kcal_peak_to_loss = `FI_cum_end_BW loss` - `FI_cum_end_peak obesity`,
    kcal_loss_to_maint = `FI_cum_end_BW maintenance` - `FI_cum_end_BW loss`,
    kcal_maint_to_regain = `FI_cum_end_BW regain` - `FI_cum_end_BW maintenance`,
    
    days_baseline_to_peak = `days_in_phase_peak obesity` + `days_in_phase_baseline`,
    days_peak_to_loss = `days_in_phase_BW loss` + `days_in_phase_peak obesity`,
    days_loss_to_maint = `days_in_phase_BW maintenance` + `days_in_phase_BW loss`,
    days_maint_to_regain = `days_in_phase_BW regain` + `days_in_phase_BW maintenance`
  )

FI_stage_summary #now we can plot this

#LM chat: convert to long format
FI_stage_long <- FI_stage_summary %>%
  select(
    ID, STRAIN, DRUG, GROUP,
    starts_with("kcal_"),
    starts_with("days_")
  ) %>%
  pivot_longer(
    cols = matches("^(kcal|days)_"),
    names_to = c(".value", "transition"),
    names_pattern = "^(kcal|days)_(.*)$"
  ) %>%
  mutate(
    kcal_per_day = kcal / days,
    transition = factor(
      transition,
      levels = c("baseline_to_peak",
                 "peak_to_loss",
                 "loss_to_maint",
                 "maint_to_regain"),
      labels = c("Baseline → Peak Obesity",
...
#LM chat 
ggplot(FI_stage_long, 
       aes(x = transition, y = kcal_per_day, color = GROUP)) +
      stat_summary(
        fun = mean, 
        geom = "point", 
        size = 3, 
