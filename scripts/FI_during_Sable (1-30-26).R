...
#This chunk of code processes the master df, sable_dwn, 
    #to creat the df sable_FI_data 
...
#### Need to UPDATE method 2 to include maintenance period too ####
#Method 2: Average FI (g) across all 4 days of Sable recording for each weight cycled mouse
  #at BW loss and BW maintenance time points
Manual_FI <- FI_manual_SABLE %>%
  filter(GROUP == "Weight cycled") %>%
  filter(SABLE == "BW loss") %>% #includes all 4 Sable days for BW loss and BW maintenance
  ungroup() %>%
  group_by(ID) %>%
...
#Wrote this section on 1-30-26
    #df1= Manual_FI_complete_days (BW loss and maintenance for weight cycled mice)
