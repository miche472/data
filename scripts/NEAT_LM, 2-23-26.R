...
# Calculate NEAT and use this method to calculate TEE (i.e. alternative way to calculate TEE)

#Revised: 2-23-26
#NOTE: 12-19-25 revision was removing 3711...see if it changes 
#any of the conclusions...this seems like an outlier because we had to re-run the 
#BW loss SABLE and therefore it was in the peak obesity to BW loss time period 
#for way longer than the other mice (~14.5 weeks)
...
ID_NEAT <- filter_locom_energy %>%
  ...
  mutate(EE_per_min= (Kcal_Hr/60)) %>% #interpolate EE per minute from EE per hr
...
GROUP_TEE <- filter_locom_energy %>%
  ungroup() %>%
  arrange(DateTime) %>%     #Make sure rows are in time order. Not strictly necessary, but maintains consistentcy with NEAT calculation
  group_by(SABLE, ID, GROUP) %>%
  mutate(EE_per_min= (Kcal_Hr/60)) %>%
...
# Left join echoMRI_data to TEE_NEAT --> combined df= TEE_NEAT_adj
...
barplot_emm_TEE_NEAT_lean <- emm_TEE_NEAT_lean_df %>%
  ...
  theme_minimal() +
  labs(
    title = "Total energy expenditure (AllMeters method) adjusted for lean mass",
...
#### Build linear mixed model for TEE (adjusted by BW) ####
  #Use BW from manual BW measurement that occurred on the relevant sable day (get from Sable_BW)
...
barplot_emm_TEE_NEAT_BW <- emm_TEE_NEAT_BW_df %>%
  ggplot(aes(x = SABLE, y = emmean, fill = GROUP)) +
  # mean bars
  geom_col(position = position_dodge(width = 0.8),
           color = "black", width = 0.7, alpha = 0.7) +
  ...
  theme_minimal() +
  labs(
    title = "Total energy expenditure (AllMeters method) adjusted for manual BW",
    y = "Adjusted TEE (kcal/day)",
    x = "Time point",
    fill = "Treatment group") +
  format.plot +
  theme(
    ...
    plot.title = element_text(hjust = 0.5),
    axis.text.x = element_text(angle = 45, hjust = 1))
barplot_emm_TEE_NEAT_BW




#### Build linear mixed model for NEAT (adjusted by lean) ####
model_NEAT_lean <- lmer(NEAT_teske ~ SABLE * GROUP + Lean + (1 | ID), data = TEE_NEAT_adj)
...
# Convert to a data frame
contrasts_NEAT_lean_df <- as.data.frame(contrasts_by_group_NEAT_lean)

#Filter for significant contrasts
Sig_contrasts_NEAT_lean <- contrasts_NEAT_lean_df %>%
  filter(p.value <=0.05)
Sig_contrasts_NEAT_lean
...
barplot_emm_NEAT_lean <- emm_NEAT_lean_df %>%
  ...
  theme_minimal() +
  labs(
    title = "NEAT (AllMeters method) adjusted for lean mass",
...
#### Build linear mixed model for NEAT (adjusted by BW) ####
#Using BW from manual BW measurement that occurred on the relevant sable day (get from Sable_BW)
#Script to create the df Sable_BW is at bottom of this script (extracted 12-19-25)
Manual_BW <- Sable_BW %>%
  mutate(ID = as.factor(ID)) %>%
  filter(!ID %in% c(3709, 3711, 3717, 3718, 3723, 3724, 3725)) %>% #3709, 3717, 3718, 3723, 3725 has cage5 issues and 3724 cage 6 is was not registered correctly
...
#### Build linear mixed model for TEE (adjusted by manually measured BW) ####
model_NEAT_BW <- lmer(NEAT_teske ~ SABLE * GROUP + Manual_BW + (1 | ID), data = TEE_NEAT_adj_manualBW)
...
# Convert to a data frame
contrasts_NEAT_BW_df <- as.data.frame(contrasts_by_group_NEAT_BW)

#Filter for significant contrasts
Sig_contrasts_NEAT_BW <- contrasts_NEAT_BW_df %>%
  filter(p.value <=0.05)
Sig_contrasts_NEAT_BW
...
barplot_emm_NEAT_BW <- emm_NEAT_BW_df %>%
  ...
  theme_minimal() +
  labs(
    title = "NEAT (AllMeters method) adjusted for manual BW",
    y = "Adjusted NEAT (kcal/day)",
    x = "Time point",
    fill = "Treatment group") +
  format.plot +
