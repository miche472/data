#Incretin agonist Experiment 2B --> try different dose escalation for TZP ####
#Objective: monitor change in BW & FI during BW loss

#Note that this was an extension of my original experiment 2 in which I had vehicle and TZP mice
#The TZP mice in the original experiment had 1 week of 10nmol/kg and a second week of 20nmol/kg.
#The mice in this. experiment had 1 week of 10nmol/kg and 1 week of 15mol/kg. I didn't
#have any control mice in this experiment since I was using the mice 
#that had received vehicle in Exp 2A for this study (i.e. for the impromptu exp 2B)

#Modified version of Exp2. tracker (Rev 6-24-26).R
#started script: 6-25-26
#revised script: 7-16-26
...
Exp2B_tracker <- BW_FI_19 %>%
  ...
  mutate( 
    ...
    STAGE = case_when(ID %in% c(3744, 3745, 3748) & DATE< "2026-06-21" ~ "BW gain",
                      ID %in% c(3744, 3745, 3748) & DATE> "2026-06-20" & DATE<"2026-07-05" ~ "BW loss", 
                      ID %in% c(3744, 3745, 3748) & DATE > "2026-07-04" ~ "BW regain", 
                      
                      ID %in% c(3746, 3752) & DATE< "2026-06-24" ~ "BW gain",
                      ID %in% c(3746, 3752) & DATE> "2026-06-23" & DATE<"2026-07-08" ~ "BW loss",
                      ID %in% c(3746, 3752) & DATE>"2026-07-07" ~ "BW regain"), 
    
    DRUG = case_when(ID %in% c(3744, 3745, 3746, 3748, 3752) ~ "Tirzepatide"),
    # All days since treatment started (including days w/o injection)
    Treatment_day =case_when(COMMENTS=="INJECT_DAY_1_10NMOL_KG"~0,
                             COMMENTS=="INJECT_DAY_2_10NMOL_KG"~1,
                             COMMENTS=="INJECT_DAY_3_10NMOL_KG"~2,
                             COMMENTS=="INJECT_DAY_4_10NMOL_KG"~3,
                             COMMENTS=="INJECT_DAY_5_10NMOL_KG"~4,
                             COMMENTS=="INJECT_DAY_6_10NMOL_KG"~5,
                             COMMENTS=="INJECT_DAY_7_10NMOL_KG"~6,
                             
                             COMMENTS=="INJECT_DAY_1_15NMOL_KG"~7,
                             COMMENTS=="INJECT_DAY_2_15NMOL_KG"~8,
                             COMMENTS=="SABLE_DAY_1_INJECT_DAY_3_15NMOL_KG"~9,
                             COMMENTS=="SABLE_DAY_2_INJECT_DAY_4_15NMOL_KG"~10,
                             COMMENTS=="SABLE_DAY_3_INJECT_DAY_5_15NMOL_KG"~11,
                             COMMENTS=="SABLE_DAY_4_INJECT_DAY_6_15NMOL_KG"~12,
                             COMMENTS=="SABLE_DAY_5_INJECT_DAY_7_15NMOL_KG"~13,
                             
                             COMMENTS=="SABLE_DAY_6_REGAIN_DAY_1"~14,
                             COMMENTS=="REGAIN_DAY_2"~15,
                             COMMENTS=="REGAIN_DAY_3"~16,
                             COMMENTS=="REGAIN_DAY_4"~17,
                             COMMENTS=="REGAIN_DAY_5"~18,
                             COMMENTS=="REGAIN_DAY_6"~19,
                             COMMENTS=="REGAIN_DAY_7"~20,
                             COMMENTS=="REGAIN_DAY_8"~21,
                             COMMENTS=="REGAIN_DAY_9"~22,
                             COMMENTS=="REGAIN_DAY_10"~24,
                             COMMENTS=="REGAIN_DAY_11"~25,
                             COMMENTS=="REGAIN_DAY_12"~26,
                             COMMENTS=="REGAIN_DAY_13"~27,
                             COMMENTS=="REGAIN_DAY_14"~28,
                             COMMENTS=="REGAIN_DAY_15"~29), 
    Dose_step = case_when(
      ID %in% c(3744, 3745, 3748) & STAGE == "BW loss" & 
        DATE > "2026-06-20" & DATE < "2026-06-28" & DRUG=="Tirzepatide" ~ "10nmol/kg",
      ID %in% c(3744, 3745, 3748) & STAGE == "BW loss" & 
        DATE > "2026-06-27" & DATE < "2026-07-05" & DRUG == "Tirzepatide" ~ "15nmol/kg",
      
      ID %in% c(3746, 3752) & STAGE == "BW loss" & 
        DATE > "2026-06-23" & DATE < "2026-07-01" & DRUG=="Tirzepatide" ~ "10nmol/kg",
      ID %in% c(3746, 3752) & STAGE == "BW loss" & 
        DATE > "2026-06-30" & DATE < "2026-07-08" & DRUG == "Tirzepatide" ~ "15nmol/kg")) %>%
  mutate(DRUG = as.factor(DRUG))

#Calculate % change in BW and cumulative intake ####
Exp2B_BWloss_plot <- Exp2B_tracker %>%
  filter(ID %in% c(3744, 3745, 3746, 3748, 3752)) %>% #only IDs undergoing injections
...
## BW (g) ####
ggplot(Exp2B_BWloss_plot, aes(x = Treatment_day, y = BW, fill = DRUG, color=DRUG)) +
  ...
  format.plot+
  #facet_wrap(~ID)+
 geom_vline(xintercept=6, linetype="dotted", linewidth=1, color="#62748E")+ #increased dose 10->15
 geom_vline(xintercept=13, linetype="solid", linewidth=1, color="#62748E")+ #final injection
  labs(x="Day",
       y= "Body weight (g)",
       title= "Exp 2B: Body weight during BW loss and BW regain",
       color="Treatment", fill="Treatment") 

#---
##USE - BW (g) by DRUG during BW loss ####
Exp2B_plot_BW_BWloss<-ggplot(Exp2B_BWloss_plot, aes(x=Treatment_day, y=BW, group=DRUG, fill=DRUG, color=DRUG)) +
  ...
  labs(x="Day",
       y= "Body weight (g)",
       title= "Exp 2B: Body weight during weight loss",
       color="Treatment", fill="Treatment")
Exp2B_plot_BW_BWloss

#Export plot to Dose response folder
ggsave(Exp2B_plot_BW_BWloss,
       filename="Exp2B_TZP_BW_plot.png", 
       width = 10, 
       height = 6, 
       units = "in", 
       dpi = 300,
...
Exp2B_plot_pct_BW_BWloss<-ggplot(Exp2B_BWloss_plot, aes(x=Treatment_day, y=BW_pct_change, group=DRUG, fill=DRUG, color=DRUG)) +
  geom_point(stat = "summary", fun = "mean", aes(color=DRUG), size=4) +
  stat_summary(fun = mean,geom = "label",aes(label = round(after_stat(y), 1), color=DRUG), vjust = -2.2, size = 4, 
               fill = "white", fontface="bold", label.padding = unit(0.15, "lines"), show.legend=FALSE) +
  ...
  labs(x="Day",
       y= "ΔBody weight (%) ",
       title= "Exp2B: Percent ΔBW during BW loss",
       color="Treatment", fill="Treatment")
Exp2B_plot_pct_BW_BWloss

#Export plot to Dose response folder
ggsave(Exp2B_plot_pct_BW_BWloss,
       filename="Exp2B_Incretin_BW_pct_plot.png", 
       width = 10, 
       height = 6, 
       units = "in", 
       dpi = 300,
...
# FI - GRAPHS ####
##Daily FI (g) ####
ggplot(Exp2B_BWloss_plot, aes(x=Treatment_day, y=corrected_intake_gr, group=DRUG, fill=DRUG, color=DRUG)) +
  geom_point(stat = "summary", 
             fun = "mean", aes(color=DRUG), size=4) +
  #stat_summary(fun = mean,geom = "label",aes(label = round(after_stat(y), 1), color=DRUG), vjust = -1, size = 4, 
  #fill = "white", fontface="bold", label.padding = unit(0.15, "lines"), show.legend=FALSE) +
  ...
  labs(x="Day",
       y= "Food intake (g/day)",
       title= "Exp 2B: Daily food intake during BW loss",
       color="Treatment", fill="Treatment")

##USE - Daily FI (kcal) ####
Exp2B_FI_daily_kcal_plot<-ggplot(Exp2B_BWloss_plot, aes(x=Treatment_day, y=corrected_intake_kcal, group=DRUG, fill=DRUG, color=DRUG)) +
  geom_point(stat = "summary", fun = "mean", aes(color=DRUG), size=4) +
  #stat_summary(fun = mean,geom = "label",aes(label = round(after_stat(y), 1), color=DRUG), vjust = -1, size = 4, 
  #fill = "white", fontface="bold", label.padding = unit(0.15, "lines"), show.legend=FALSE) +
  geom_line(stat = "summary", fun = "mean", aes(color=DRUG), linewidth=1.5) +
  ...
  labs(x="Day",
       ...
       title= "Exp 2B: Daily food intake during BW loss",
       color="Treatment", fill="Treatment")
Exp2B_FI_daily_kcal_plot

#Export plot to Dose response folder
ggsave(Exp2B_FI_daily_kcal_plot,
       filename="Exp2B_FI_daily_kcal_plot.png", 
       width = 10, 
       height = 6, 
       units = "in", 
       dpi = 300,
...
Exp2B_FI_cum_kcal_plot<-ggplot(Exp2B_BWloss_plot, aes(x = Treatment_day, y = BWloss_cum_INTAKE_kcal, color = DRUG, fill = DRUG)) +
  geom_point(stat = "summary", 
  ...
  labs(x="Day",
       ...
       title= "Exp 2B: Cumulative food intake during BW loss",
       color="Treatment",
       fill="Treatment")
Exp2B_FI_cum_kcal_plot

#Export plot to Dose response folder
ggsave(Exp2B_FI_cum_kcal_plot,
       filename="Exp2B_FI_cum_kcal_plot.png", 
       width = 10, 
       height = 6, 
       units = "in", 
       dpi = 300,
...
##CREATE df ####
Exp2B_manual_check <- Exp2B_BWloss_plot %>%
...
##GRAPH manual check ####
ggplot(Exp2B_manual_check, aes(x = Treatment_day)) +
  ...
  labs(x="Day",
       title= "Exp2B: Trajectory of FI & BW loss (NZO, n=12)",
       y="ΔBW (%) & Daily FI (g)") +
  facet_wrap(~ID)

###Graph just TZP ####
Exp2B_TZP_manual_check <- Exp2B_manual_check %>%
  filter(DRUG=="Tirzepatide")

ggplot(Exp2B_TZP_manual_check, aes(x = Treatment_day)) +
  geom_line(aes(y = BW_pct_change, color = "BW (% change)"), color="darkviolet") +
  geom_point(aes(y = BW_pct_change, color = "BW (% change)"), color="darkviolet") +
  geom_label(aes(y = BW_pct_change, label = round(BW_pct_change,1), nudge_y = -4, 
                 color="BW (% change)"), size=3, color="darkviolet") +
  ...
  theme(legend.position = "right",
        ...
        legend.text = element_text(size = 14)) +
  labs(x="Day",
       title= "Exp 2B: Tirzepatide: FI & BW loss (NZO, n=5)",
       y="ΔBW (%) & Daily FI (g)") +
  facet_wrap(~ID)
...
## Tirzepatide ####
TZP_Exp2B_BWloss_plot <- Exp2B_BWloss_plot %>%
  filter(DRUG=="Tirzepatide")

### BW TZP ####
Exp2B_TZP_plot_pct_BW_BWloss<-ggplot(TZP_Exp2B_BWloss_plot, aes(x=Treatment_day, y=BW_pct_change, group=DRUG, fill=DRUG, color=DRUG)) +
  geom_point(stat = "summary", fun = "mean", aes(color=DRUG), size=4) +
  ...
  labs(x="Day",
       y= "ΔBW (%) ",
       title= "Exp2B: Tirzepatide: ΔBW (%)",
       color="Treatment", fill="Treatment")
Exp2B_TZP_plot_pct_BW_BWloss

### FI TZP ####
####TZP FI --> grams ####
Exp2B_TZP_FI_daily_gr_plot<-ggplot(TZP_Exp2B_BWloss_plot, aes(x=Treatment_day, y=corrected_intake_gr, group=DRUG, fill=DRUG, color=DRUG)) +
  geom_point(stat = "summary", fun = "mean", aes(color=DRUG), size=4) +
  stat_summary(fun = mean,geom = "label",aes(label = round(after_stat(y), 1)), vjust = -3.3, size = 4, color = "black", 
               fill = "white", fontface="bold", label.padding = unit(0.15, "lines")) +
  geom_line(stat = "summary", fun = "mean", aes(color=DRUG), linewidth=2) +
  ...
  labs(x="Day",
       ...
       title= "Exp 2B: Tirzepatide: Daily food intake (g)",
       color="Treatment", fill="Treatment")
Exp2B_TZP_FI_daily_gr_plot

#### TZP FI --> kcal ####
Exp2B_TZP_FI_daily_kcal_plot<-ggplot(TZP_Exp2B_BWloss_plot, aes(x=Treatment_day, y=corrected_intake_kcal, group=DRUG, fill=DRUG, color=DRUG)) +
  geom_point(stat = "summary", fun = "mean", aes(color=DRUG), size=4) +
  stat_summary(fun = mean,geom = "label",aes(label = round(after_stat(y), 1)), vjust = -3.3, size = 4, color = "black", 
               fill = "white", fontface="bold", label.padding = unit(0.15, "lines")) +
  geom_line(stat = "summary", fun = "mean", aes(color=DRUG), linewidth=2) +
  ...
  labs(x="Day",
       y= "Food intake (kcal/day)",
       title= "Exp 2B: Tirzepatide: Daily food intake (kcal)",
       color="Treatment", fill="Treatment")
Exp2B_TZP_FI_daily_kcal_plot