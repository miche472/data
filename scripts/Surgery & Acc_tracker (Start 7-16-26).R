...
post_surgery_tracker <- BW_FI_19 %>% 
  ...
  mutate(
    STAGE = case_when(day_rel_surg ==0 ~ "Surgery",
                      ...
                      DATE=="2026-06-30" ~ "Day 1 sable acclimation",
                        
                      DATE=="2026-07-06" ~ "First mock",
                      DATE=="2026-07-09" ~ "Second mock",
                      DATE=="2026-07-12" ~ "Third mock",
                      DATE=="2026-07-15" ~ "Fourth mock"))
...
#Sable acclimation and mock injections ####
sable_acc_mock_tracker <- BW_FI_19 %>% 
  arrange(DATE) %>% 
  filter(ID %in% c(3731, 3732, 3733, 3735, 3737, 3738, 3739, 3740, 3741)) %>%
  group_by(ID) %>% 
  filter(DATE >= "2026-06-30") %>% #Day sable acclimation started
  mutate(day_rel_sable_acc = as.integer(as.Date(DATE) - as.Date(first(DATE)))) %>%
  ungroup() %>%
  group_by(ID) %>%
  arrange(DATE) %>%
  mutate(INTAKE_GR = if_else(INTAKE_GR >= 0, INTAKE_GR, 0, missing=0),
  ...
  group_by(ID) %>%
  arrange(DATE) %>%
  mutate(BW_pct_change = 100*((BW - first(BW)) / first(BW))) #Calculate percent change in BW since sable acclimation started
        

# Body weight change --> facet wrap graphs
## Graph BW ####
ggplot(sable_acc_mock_tracker, aes(x = day_rel_sable_acc, y = BW_pct_change)) +
  geom_line(aes(y = BW_pct_change, group = ID)) +
  geom_point() +
  theme_bw(base_size = 14) +
  format.plot_LM+
  geom_vline(xintercept=6, linetype="dotted", linewidth=1, color="#62748E")+ #1st mock icv injection
  geom_vline(xintercept=9, linetype="dotted", linewidth=1, color="#62748E")+ #2nd mock icv injection
  geom_vline(xintercept=12, linetype="dotted", linewidth=1, color="#62748E")+ #3rd mock icv injection
  geom_vline(xintercept=15, linetype="dotted", linewidth=1, color="#62748E")+ #4th mock icv injection
  geom_hline(yintercept=0)+
  facet_wrap(~ID)+
  labs(x="Day of acclimation",
       y= "ΔBody weight (%)",
       title= "ΔBW (%) acclimation & mock injection") 
