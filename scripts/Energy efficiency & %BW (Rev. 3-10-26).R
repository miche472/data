...
ggplot(alt_efficiency, aes(x = Transition, y = phase_FI_BW, fill = GROUP)) +
      ...
      theme(
        ...
        strip.text = element_text(face = "bold", size = 12)) +
      labs(
        title="Alt. energy efficiency (Cumulative intake/ΔBW)",
        x = "Stage of weight cycle",
...
#Statistical analysis for % change in BW ####
...
# (Use) FI graph (raw values) -> during the stages/transition periods ####
    ...
    format.plot+
    labs(
      title="Daily food intake (kcal/day)",
      x = "Stage of weight cycle",
      y = "Food intake (kcal/day) during stage",
      fill = "Treatment group") +
  theme(legend.position = "right",
        plot.title = element_text(size=16, hjust = 0.5, face="bold"),
        legend.title = element_text(size = 12, face="bold"),
