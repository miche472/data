...
filter_loc <-sable_dwn %>%
  ...
  mutate(
    ...
    complete_days = cumsum(if_else(zt_time==0 & is_zt_init == 1,1,0))) %>% 
  ungroup() %>% 
  group_by(ID, complete_days) %>% # I think that grouping by date at this point would help to not have duplicates for hours...complete day can span multiple dates though...it seems like I need to 
  #arrange by date somehow? Could 
  mutate(is_complete_day = if_else(min(zt_time)==0 & max(zt_time)==23, 1, 0)) %>% 
  ungroup() %>%
  group_by(ID, complete_days, is_complete_day, SABLE) %>% 
  filter(!ID %in% c(3715,3712)) %>% #remove mice that died during study
...
#TO get complete day 1, try explicitly stating what zt time is and what sable day is
#taken from original
filter_loc <-sable_dwn %>%
  filter(COHORT %in% c(3, 4, 5)) %>%   
  mutate(lights  = if_else(hr %in% c(20,21,22,23,0,1,2,3,4,5), "off", "on")) %>% 
  ...
  mutate(
    ...
    complete_days = cumsum(if_else(zt_time==0 & is_zt_init == 1,1,0))) %>% 
  ungroup() %>%
group_by(ID) #in original version she also grouped by SABLE here
#end of portion taken from original
#My method to identify complete day 1 during each sable period
filter_loc2 <- filter_loc %>%
