...
##Graphs (BW) ####
###Graph BW (emmeans) ####
plot_emm_delta_BW_g_stage <-ggplot(emm_change_BW_g_df, aes(x = Transition, y = emmean, fill = GROUP))  +
...
ggsave(plot_emm_delta_BW_g_stage,
       filename="emm_delta_BW_g_stage.png", 
       width = 6, 
       height = 6, 
       units = "in", 
       dpi = 300,
...
###Graph BW for ONLY Obesity development & Weight regain ####
df4_plot_delta_BW_g_stage <- df4 %>%
  filter(Transition %in% c("BW gain", "BW regain"))

two_stage_plot_delta_BW_g_stage <- ggplot(df4_plot_delta_BW_g_stage, aes(x = Transition, y = delta_BW_g, fill = GROUP, color=GROUP)) +
  geom_bar(stat = "summary", 
           fun = "mean", 
           position = position_dodge(width = 0.8), width=0.73) +
  geom_errorbar(stat = "summary", 
  ...
  labs(
    #title="Change in body weight (g)",
    y = "ΔBW (g)")
two_stage_plot_delta_BW_g_stage

#Export plot to folder called "SSIB_2026" 
ggsave(two_stage_plot_delta_BW_g_stage,
       filename="Raw_delta_BW_g_two_stages.png", 
       width = 6, 
       height = 6, 
       units = "in", 
       dpi = 300,
...
plot_emm_FI_daily_stage <- ggplot(emm_FI_daily_stage_df, aes(x = Transition, y = emmean, fill = GROUP)) +
  ...
  format.plot +
labs(
  #title = "Daily energy intake (kcal/day)",
...
plot_daily_FI_stage <- ggplot(df4, aes(x = Transition, y = Daily_kcal, fill = GROUP, color=GROUP)) +
  ...
  theme(#legend.position = "right",
    ...
    legend.title=element_blank(),
    legend.text=element_blank(),
    axis.text.x = element_text(size= 20, color="black", hjust=1, vjust=1, angle=20),
    axis.title.x = element_blank(),
    axis.title.y = element_text(size= 20, face="bold"),
    axis.text.y = element_text(size = 20),
    panel.border = element_blank()) +
...
#Build multiple linear regression model
model_energy_efficient <- lmer(phase_BW_FI ~ Transition * GROUP + (1 | ID), data = df4)
summary(model_energy_efficient)

# Calculate estimated marginal means 
emm_energy_efficient <- emmeans(model_energy_efficient, ~ Transition * GROUP, cov.reduce = mean)
emm_energy_efficient_df <- as.data.frame(emm_energy_efficient)

# Pairwise contrasts within each GROUP
contrasts_by_group_energy_efficient <- contrast(emm_energy_efficient, method = "pairwise", by = "GROUP")
contrasts_by_group_energy_efficient_df <- as.data.frame(contrasts_by_group_energy_efficient)

# Pairwise contrasts within each stage of weight cycling
contrasts_by_SABLE_energy_efficient <- contrast(emm_energy_efficient, method = "pairwise", by = "Transition")
contrasts_SABLE_energy_efficient_df <- as.data.frame(contrasts_by_SABLE_energy_efficient)

##Graphs (Energy efficiency) ####
###Graph Energy efficiency (emmeans) ####
emm_energy_efficient_2_stages_df <- emm_energy_efficient_df %>%
filter(Transition %in% c("BW gain", "BW regain"))

plot_emm_energy_efficient <-ggplot(emm_energy_efficient_2_stages_df, aes(x = Transition, y = emmean, fill = GROUP))  +
  geom_col(position = position_dodge(width = 0.8), width = 0.73) +
  geom_errorbar(aes(ymin = emmean - SE, ymax = emmean + SE),
                position = position_dodge(width = 0.8),
                width = 0.25, linewidth = 0.75, color="#454441") +
  ...
  geom_hline(yintercept = 0, color = "black", linewidth = 0.7, linetype = "solid") +
  labs(
    #title = "Energy efficiency (ΔBW/Energy intake)",
       y = "Energy efficiency (ΔBW/Energy intake)")
plot_emm_energy_efficient
#Conclusion: Energy efficiency during BW regain stage is 6.28x10^-17 
#for weight cycled vs control (linear mixed model, estimated marginal means, p=6.28x10^-17 )

#Export plot to folder called "APS_figures" 
ggsave(plot_emm_energy_efficient,
       filename="emm_energy_efficient.png", 
       width = 6, 
       height = 6, 
       units = "in", 
       dpi = 300,
...
#Graph Energy Efficiency for ONLY Obesity development & Weight regain
df4_efficient <- df4 %>%
  filter(Transition %in% c("BW gain", "BW regain"))

plot_energy_efficient_raw <- ggplot(df4_efficient, aes(x = Transition, y = phase_BW_FI, fill = GROUP, color=GROUP)) +
  geom_bar(stat = "summary", 
           fun = "mean", 
           position = position_dodge(width = 0.8), width=0.73) +
  geom_errorbar(stat = "summary", 
  ...
  labs(
    #title="Energy efficiency (ΔBW/energy intake)",
       y = "Energy efficiency (ΔBW/Energy intake)")
plot_energy_efficient_raw

#Export plot to folder called "APS_figures" 
ggsave(plot_energy_efficient_raw,
       filename="Raw_energy_efficiency_plot.png", 
       width = 6, 
       height = 6, 
       units = "in", 
       dpi = 300,
...
#Create legend (with points) ####
plot_legend_points <- ggplot(df4_efficient, aes(x = Transition, y = phase_BW_FI, fill = GROUP, color=GROUP)) +
  geom_bar(stat = "summary", 
           fun = "mean", 
           position = position_dodge(width = 0.8), width=0.73) +
  geom_errorbar(stat = "summary", 
  ...
  format.plot+
  geom_hline(yintercept = 0, color = "black", linewidth = 0.7, linetype = "solid") +
  labs(title="Energy efficiency (ΔBW/energy intake)",
       y = "ΔBW/energy intake (g/kcal)",
       fill="Treatment",
       color="Treatment")
plot_legend_points

ggsave(plot_legend_points,
       filename="plot_legend_points.png", 
       width = 6, 
       height = 6, 
       units = "in", 
       dpi = 300,
...
#Create legend (without points) ####
plot_legend_NO_POINTS <- ggplot(emm_FI_daily_stage_df, aes(x = Transition, y = emmean, fill = GROUP)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.73) +
  geom_errorbar(aes(ymin = emmean - SE, ymax = emmean + SE),
                position = position_dodge(width = 0.8),
                width = 0.25, linewidth = 0.75, color="#454441") +
  ...
  labs(title="Energy efficiency (ΔBW/energy intake)",
       y = "ΔBW/energy intake (g/kcal)",
       fill="Treatment")
plot_legend_NO_POINTS

ggsave(plot_legend_NO_POINTS,
       filename="plot_legend_NO_POINTS.png", 
       width = 6, 
       height = 6, 
       units = "in", 
       dpi = 300,
