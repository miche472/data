...
#Average daily FI (kcal) recorded by Sable for each GROUP (except Weight cycled BW loss/maintenance)
Avg_Sable_FI <- sable_FI_minutes %>%
  ungroup() %>%
  group_by(ID, SABLE) %>%
  filter(!(SABLE == "BW regain" & ID == "3720")) %>% #remove visually apparent outliers.3713 at Baseline (kcal=72.2) and 3720 at BW regain (kcal=53.6)
...
#### Calculate FI during BW loss/maintenance Sable recording for Weight cycled mice with manual measurements ####
...
#Create df "FI_manual_SABLE" -> Process FI.csv file (manually measured FI)
    #Will allow for extraction of FI for weight cycled mice during BW loss/maintenance
...
#Use "FI_manual_SABLE" to calculate manual FI during BW loss & maintenance  -> Two possible methods to calclate avg FI (daily)
  #Two methods: use 2 middle days (Manual_FI_complete_days) or all 4 days (Manual_FI)
...
Manual_FI_complete_days <- FI_manual_SABLE %>%
  ...
  filter(sable_idx %in% c("SABLE_DAY_SABLE_DAY_13", "SABLE_DAY_14", "SABLE_DAY_17", "SABLE_DAY_18")) %>%
  ungroup() %>%
...
##### COMPILED df with FI for control and weight cycled mice during all Sable time points ####
    ...
    # Method 1: Manual FI for all Sable days (avg daily intake) for BW loss and BW maintenance for weight cycled mice
        #sable recorded data is complete days 1 and 2 at each time point
    FI_in_Sable_4 <- bind_rows(sable_FI_minutes, Manual_FI)
    
    # Method 2: Manual FI for the two middle Sable recording days for BW loss and BW maintenance for weight cycled mice
        #sable recorded data is complete days 1 and 2 at each time point
...
#Is FI significantly different when using the 2 days vs 4 day approach? 
    #Based on the graphs coded below, this doesn't make a very noticeable difference
...
#Graph FI calculated using only avg of FI during complete days 1 and 2
    #for weight cycled during BW loss/maintenance (FI_in_Sable_4)
ggplot(FI_in_Sable, aes(x = SABLE, y = avg_corrected_intake_kcal, fill = GROUP)) +
    ...
    scale_fill_manual(values = custom_colors) +
      labs(
      title= "Average daily food intake during Sable recording (complete days)",
        x = "Time point",
        y = "Averageg daily food intake (kcal/day)",
...
#Graph FI calculated using avg FI during all 4 days of Sable 
  #for weight cycled during BW loss/maintenance (FI_in_Sable_4)
ggplot(FI_in_Sable_4, aes(x = SABLE, y = avg_corrected_intake_kcal, fill = GROUP)) +
  geom_bar(
    stat = "summary",
    fun = "mean",
    position = position_dodge(width = 0.8),
  ...
  scale_fill_manual(values = custom_colors) +
  labs(
    title= "Average daily food intake during Sable recording (all days)",
    x = "Time point",
...
#UPDATE : this is already located in the code from 1-29-26 ####
#Complete days only. df called FI_in_Sable (also created above to allow for graphing)
  FI_in_Sable <- bind_rows(sable_FI_minutes, Manual_FI_complete_days)

  
  #Average of two complete days for each mouse at each time point
  #average and stdev
...
# 1. Check for outliers in FI during Sable recording
    #Graph boxplots
    library("ggpubr")
    #All four recording days
    ggboxplot(FI_in_Sable, x = "SABLE", y = "avg_corrected_intake_kcal", color = "GROUP",
              palette = c("#FAAC41", "#3498DB"))
    
    #All four recording days
    ggboxplot(FI_in_Sable_4, x = "SABLE", y = "avg_corrected_intake_kcal", color = "GROUP",
              palette = c("#FAAC41", "#3498DB"))
...
#####Graph the manual FI measurements during Sable recording ####
    #Create data frame with manual FI values for all mice at all time points during
    #Sable recording (i.e. use manual rather than FI measured by sable)
      
#DELETE --> Method 1: Average FI (g) for the two middle days of recording for all mice
  #This is similar to the concept of including only complete days 1 and 2 for FI measured by Sable
  #This method doesn't work for manual measurements since they weren't taken on consecutive days
      Manual_FI_all_mice_complete_days <- FI_manual_SABLE %>%
      mutate(ID = factor(ID)) %>%
      filter(sable_idx %in% c(
        "SABLE_DAY_2","SABLE_DAY_3","SABLE_DAY_9","SABLE_DAY_10",
        "SABLE_DAY_13","SABLE_DAY_14","SABLE_DAY_17","SABLE_DAY_18", 
        "SABLE_DAY_20","SABLE_DAY_21")) %>%
      ungroup() %>%
      group_by(ID, SABLE) %>%
      summarise(avg_corrected_intake_kcal = mean(corrected_intake_kcal))
    
    #(*Only method) Method 2: Average daily FI (g) across duration (4 days) of Sable recording for all mice
...
# 1.  Energy balance (left join data frames below to facilitate)
      # EE (sable_TEE_adj_RMR) - FI, sable recorded (FI_in_Sable_4)
      # EE (sable_TEE_adj_RMR) - FI, manual for all mice (Manual_FI_all_mice) 
...
# start of possible delete ####
#This chunk of code processes sable_dwn to sable_FI_data 
#sable_FI_data only has FoodA under the value column and only includes complete days 1 and 2

#Modify order of steps in processing of sable_dwn to more closely match the approach for EE. Is the outcome the same? ####

sable_FI_data2 <- sable_dwn %>% 
  filter(COHORT > 2 & COHORT < 6) %>% # Just NZO
  mutate(
    lights = if_else(hr %in% c(20,21,22,23,0,1,2,3,4,5), "Dark", "Light"),
    SABLE = case_when(
...
#Sum the mass of food eaten each minute to get total food eaten on complete days 1 and 2
#sable_FI_minutes excludes mice that were recorded in Sable cages that were problematic
sable_FI_minutes2 <- sable_FI_data2 %>%
  group_by(ID, complete_days, DRUG, GROUP, SABLE) %>%
...
Avg_Sable_FI2 <- sable_FI_minutes2 %>%
  ungroup() %>%
  group_by(SABLE, GROUP) %>%
  summarise(GROUP_avg_corrected_intake_gr= mean(avg_corrected_intake_gr), .groups="drop")

#Stop working on FI for a moment and look at energy balance. this may clarify which FI measures are more accurate
    #If not, look at physical copies of manual FI recording
#filter Manual_FI_gr_kcal and FI_in_Sable_gr_kcal to remove mice with problematic Sable recordings
#Use left join to combine TEE/RMR data frame and each of the two FI data frames

#Join TE

#UPDATE: use this to remove problematic mice from combine FI and EE dfs
#Manual_FI_gr_kcal (remove problematic cages and the two FI outliers)
filtered_Manual_FI_gr_kcal <- Manual_FI_gr_kcal %>%
  ungroup() %>%
  group_by(ID) %>%
  filter(!ID %in% c(3709, 3717, 3718, 3723, 3724, 3725)) %>%
  #ungroup() %>%
...
TEE_manual_FI <- sable_TEE_adj_RMR %>%
  left_join(
    filtered_Manual_FI_gr_kcal %>% 
      select(ID, SABLE, GROUP, avg_corrected_intake_gr, KCAL_G, avg_corrected_intake_kcal),
    by = c("ID", "SABLE", "GROUP")
  ) %>%
  ungroup() %>%
  group_by(ID, SABLE) %>%
  filter(!(SABLE == "BW regain" & ID == "3720")) %>% #remove visually apparent outliers.3713 at Baseline (kcal=72.2) and 3720 at BW regain (kcal=53.6)
  filter(!(ID == "3713" & SABLE == "Baseline"))  %>%
  mutate(balance_kcal = avg_corrected_intake_kcal - tee)

#FI measured by Sable (using my original method to calculate sable FI + manual for WC in BW loss/maintenance)
####UPDATE: what is the name of the df that has this FI data? ####
filtered_sable_FI_minutes2 <- sable_FI_minutes2 %>%
  ungroup() %>%
  group_by(ID, SABLE, GROUP) %>%
  mutate(KCAL_G = if_else(SABLE=="Baseline", 3.1, 3.82),
...
EE_sable_FI2 <- sable_TEE_adj_RMR %>%
  left_join(
    filtered_sable_FI_minutes2 %>% 
      select(ID, SABLE, GROUP, avg_corrected_intake_gr, KCAL_G, avg_corrected_intake_kcal),
    by = c("ID", "SABLE", "GROUP")
  ) %>%
  ungroup() %>%
