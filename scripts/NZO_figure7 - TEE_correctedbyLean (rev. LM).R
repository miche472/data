#Calculate 24hr TEE in NZO mice
#Use linear mixed model to correct for lean mass and repeated measures 
...
#### Build linear mixed model for TEE (adj for lean) #### 
...
# Pairwise contrasts within each GROUP (ad lib or restricted)
      ...
      contrasts_TEE_lean_df <- as.data.frame(contrasts_by_group_TEE_lean)
      
      # Filter for restricted group and baseline vs other time points
      restricted_contrast_TEE_lean <- contrasts_TEE_lean_df %>%
        filter(GROUP == "Restricted") %>%
        filter(contrast %in% c("Baseline - BW loss", "Baseline - BW maintenance", "Baseline - BW regain"))
      restricted_contrast_TEE_lean
      
      #Filter for significant contrasts
      Sig_contrast_TEE_lean <- contrasts_TEE_lean_df %>%
        filter(p.value <=0.05)
      Sig_contrast_TEE_lean
      
# Pairwise contrasts within each SABLE (time point)
      contrasts_SABLE_by_group_TEE_lean <- contrast(emm_TEE_lean, method = "pairwise", by = "SABLE")
      # Convert to a data frame
      contrasts_SABLE_TEE_lean_df <- as.data.frame(contrasts_SABLE_by_group_TEE_lean)
      contrasts_SABLE_TEE_lean_df
...
####Scatter plot - Graph predicted TEE adjusted for lean mass (NZO) ####
...
ggplot() +
  ...
  geom_errorbar(data = emm_TEE_lean_df,
  ...
  labs(y = "Adjusted TEE (kcal/day)", x = "Time point",
       ...
       title = "Total energy expenditure (TEE) adj. for lean mass") +
  format.plot +
