...
#### move into Excel temporarily and manually add the SABLE phase that each mouse is in...this should be easy because I can use the 
      #SABLE phases already in the column as a guide
write_csv(x = FI_data_LM, "../data/FI_data_LM2.csv")
FI_data_LM2 <- read_csv("../data/FI_data_LM2.csv")
...
TEE_FI_LM <- TEE_FI_LM %>%
  ...
  mutate (Balance = sable_kcal_day - tee)
    ...
    #Energy balance for just Weight cycled mice
    restricted_balance <- TEE_FI_LM %>%
      filter(GROUP == "Weight cycled") %>%
      rename(Energy_intake = sable_kcal_day) %>%
      select(ID, SABLE, tee, Energy_intake, Balance, GROUP, DRUG)

#Summary stats (tee, intake, Balance) for Control and Weight cycled at each time point
Balance_summary <- TEE_FI_LM %>%
  ungroup() %>%
  group_by(SABLE, GROUP) %>%
  summarise(
    mean_EEbalance = mean(Balance, na.rm = TRUE),
    mean_tee = mean(tee, na.rm = TRUE),
    mean_intake = mean(sable_kcal_day, na.rm = TRUE),
    .groups = "drop")
