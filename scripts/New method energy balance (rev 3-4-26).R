...
#What if I take the sum of EE per hour and then take the mean hourly energy expenditure average hourly EE for each hour. 
#The issue with averages is that there is a lot of variation between hours, so a lot of detail is lost
#This would be fine for energy balance, except it is difficult to determine how to append the manual food...the 
#number of meals to include depends on the time of day when EE was recorded

#For each hour of recording, calculate the average of the minute by minute measurements for EE. The units will be kcal/hr. 
#For all hours of measurement, add together these values. This is a good estimate of the TEE over the 

#don't group by complete_days because this will help with the fact that I don't have measurements 
#for every hour of 2 complete days for every mouse at each time point
...
#Look for outliers in hourly energy balance
