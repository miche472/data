...
filter_FI1_revised <-sable_dwn %>%
  ...
  filter(recording_duration >= 22.5, #complete day must have a total of at least 23 hrs of data
...
#Evaluate if data is being assigned to the correct mice when two mice record in the same cage on the same day####
#looking at sable_dwn confirmed this is a problem
#Example: On 6-17-26 3743 stopped recording in cage 2 and 3753 started recording in cage 2. Code is assigning all 
#values from that day to both mice
#This artificially creates an extra complete day for 3743 since there is data for the cage through 6:30pm on 6-17-26 
#and the code doesn't realize that this data is for another mouse. How can I address this?
#Options: Always ignore the last complete day for a recording --> doesn't require tons of hardcoding
#4 days of recording results in 3 complete days if the cage continues recording after removing the original mouse. 
#After the code gives you numbered complete days filter out the max(complete_days)
...
#Now eliminate the final complete day for each ID during each SABLE period for which there are more than 2 complete days
#if there are more than 2 complete days then the final complete day is removed. If there are only 2 complete days that 
#means that the cage didn't record beyond the time when mouse A was done recording and therefore the final complete day 
#is truly data from mouse A not from mouse B or an empty cage

filter_FI_qc <- filter_FI1_revised %>%
  filter(!(cage_number=="3")) %>% #food mass monitor doesn't work in this cage
  group_by(ID, SABLE) %>%
...
FI_quant_Sable <- filter_FI_qc %>%
ungroup() %>%
  group_by(ID, SABLE) %>%
  arrange(DateTime) %>%
  mutate(diff=lag(value)-value) 

FI_quant_Sable2 <- filter_FI_qc %>%
  
  ungroup() %>%
  group_by(ID, SABLE, complete_days) %>%
  arrange(DateTime) %>%
  summarise(diff2=first(value)-last(value)) 
...
distinct(ID, SABLE, complete_days, recording_duration, zt_hours) %>%
arrange(ID, SABLE, complete_days)
