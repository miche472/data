...
loc_plotdata <- sable_locomotion_data %>%
  mutate(
    PlotGroup = case_when(
      ...
      #SABLE %in% c("baseline", "peak obesity", "BW loss", "BW maintenance") ~ GROUP,
      SABLE == "baseline" ~ paste(GROUP, DRUG, sep = "_"),
      SABLE == "peak obesity" ~ paste(GROUP, DRUG, sep = "_"),
      SABLE == "BW loss" ~ paste(GROUP, DRUG, sep = "_"),
      SABLE == "BW maintenance" ~ paste(GROUP, DRUG, sep = "_"),
...
loc_plot <- loc_plotdata %>%
  ...
  theme_minimal() +
  labs(y = "Locomotion (m/day)", fill = "Drug", color = "Group") +
