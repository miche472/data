...
#I want to take the average of the corrected daily FI (kcal) during the SABLE_DAY_x for which there is a FI measurement
...
FI_avg <- FI_data_LM %>%
  group_by(ID, SABLE) %>%
  summarise(
   sable_kcal_day = mean(corrected_intake_kcal, na.rm = TRUE),
    GROUP       = first(GROUP),
    DRUG        = first(DRUG),
    .groups = "drop")
  
# 1C. Calculate average daily FI during each of the sable measurement phases for ALL mice (divided by Control vs Weight cycled)
FI_avg_2 <- FI_data_LM %>%
  ungroup() %>%
  group_by(SABLE, GROUP) %>%
  summarise(
    Avg_SABLE_kcal_day = mean(corrected_intake_kcal, na.rm = TRUE),
    .groups = "drop")
...
#To allow for joinging with sable_TEE_adj_RMR, Change ID to factor and remove mice that Sable didn't record properly
FI_avg <- FI_avg %>%
  mutate(ID = factor(ID)) %>%
