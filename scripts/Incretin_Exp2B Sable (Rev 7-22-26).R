...
filter_loc_TEE3_exp2b <- filter_loc_TEE2_exp2b %>%
...
Minute_sum_EE_hr_all_days_exp2b <- filter_loc_TEE2_exp2b %>%
  ungroup() %>%
  arrange(DateTime) %>%
  # Movement each hr (during how many minutes did mouse move)
  group_by(SABLE, ID) %>%
...
#Need to use written data sheets to hard code when mice were removed from cages
#if meta data shows a mouse is recording in a cage on a day but the mouse is removed 
#before 6:30pm, and then the system starts recording again because other mice must be recorded
#then a complete day that isn't really a complete day is identified
#For now perhaps I shouldn't include the last complete day for any mice
#probably easier to filter FOR complete days rather than removing complete days
#that aren't realy complete days

filter_food2_exp2b <- filter_food1_exp2b %>%
  filter(!(cage_number=="3")) %>% #wasn't recording food properly
  filter(complete_days %in% c(2,3)) %>%
  ungroup()%>%
  group_by(ID, SABLE) %>%
  arrange(DateTime) %>%
